...
data/ui, data/ui/v9, ui/V9_ui_src
"""
...
# V9 UI source and built dist live under BASE_DIR/data/ui. Keeping updater,
# backend serving, and GUI launch on the same path prevents blank embedded UI
# windows and avoids writing frontend assets into the wrong project root.
...
def ensure_base_dirs() -> None:
    """
    Ensure data/ui, v9, and V9_ui_src directories exist.
    """
...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-backup",
        action="store_true",
        help="Skip backup of existing UI (data/ui/v9)."
    )
    parser.add_argument(
        "--build-script",
        default="build",
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryUIupdater.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryUIupdater',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryUIupdater', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
