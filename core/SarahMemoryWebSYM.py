...
# WebSYM static fallback normalization.  This keeps local/offline answer pools
# reachable when chat sends natural questions such as "who is Bill Gates".
# ============================================================================
def _sm_websym_query_variants(query: str) -> list:
    ...
    for pat in (
        ...
        r"^(tell\s+me\s+about|explain|define|describe|research)\s+",
        r"^(look\s+up|find\s+information\s+on|find\s+info\s+on)\s+",
    ):
        stripped = re.sub(pat, "", stripped).strip()
    stripped = re.sub(r"^(a|an|the)\s+", "", stripped).strip()
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryWebSYM.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryWebSYM', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
