...
# --- SARAHMEMORY REALITY PATCH 2026-07-23: Local Model Bootstrap + Tokenizer Profile ---
# These helpers make the thesis operational without pretending SarahMemory has
# magically trained a foundation model. The first real step is a governed local
# model install plus SarahMemory-owned tokenizer/profile instrumentation.

_SM_TOKENIZER_PROFILE_SCHEMA = "SarahMemory.tokenizer_profile.v0.1"
_SM_BOOTSTRAP_SCHEMA = "SarahMemory.local_model_bootstrap.v0.1"
_SM_FORENSICS_SCHEMA = "SarahMemory.model_forensics.v0.1"


def _sm_sha256_file(path: str, max_mb: float = 16.0) -> Optional[str]:
    try:
...
def _sm_model_artifact_hashes(local_dir: str) -> Dict[str, Any]:
    tracked = (
        "tokenizer.json", "tokenizer_config.json", "vocab.json", "merges.txt", "special_tokens_map.json",
        "config.json", "generation_config.json", "quantization_config.json", "adapter_config.json",
    )
    out: Dict[str, Any] = {"local_dir": local_dir, "artifacts": {}}
    try:
        for name in tracked:
...
def _sm_tokenizer_profile_dir() -> str:
    path = os.path.join(MODELS_DIR, "SarahMemory", "tokenizer_profiles")
    os.makedirs(path, exist_ok=True)
    return path


def _sm_basic_subtokens(text: str) -> List[str]:
    try:
        return re.findall(r"[A-Za-z]+|\d+|[^\w\s]", str(text or ""), flags=re.UNICODE)[:512]
    except Exception:
        return str(text or "").split()[:512]


def inspect_tokenizer_path(
    text: str,
    model_id: str = "",
    repo: str = "",
    max_tokens: int = 256,
) -> Dict[str, Any]:
    """Logically inspect token ids/subtokens for a local model tokenizer.

    This is read-only. If no HF tokenizer is available locally, it returns a bounded
    deterministic fallback token path so governance and UI can still inspect the request.
    """
    raw = str(text or "")
    max_tokens = max(1, min(int(max_tokens or 256), 512))
    selected_record = None
    local_dir = ""
    if model_id:
        try:
            status = get_model_manager_status(refresh=False)
            for rec in status.get("models", []) or []:
                if str(rec.get("id") or "") == str(model_id):
                    selected_record = rec
                    local_dir = str(rec.get("path") or "")
                    break
        except Exception:
            pass
    if not local_dir and repo:
        local_dir = repo_to_local_dir(resolve_model_repo(repo))
    if not local_dir:
        rec = get_active_model_record("reasoning") or get_active_model_record("chat") or {}
        if rec:
            selected_record = rec
            local_dir = str(rec.get("path") or "")

    hashes = _sm_model_artifact_hashes(local_dir) if local_dir else {"artifacts": {}}
    if AutoTokenizer is not None and local_dir and os.path.isdir(local_dir):
        try:
            tok = AutoTokenizer.from_pretrained(local_dir, local_files_only=True, trust_remote_code=False)
            encoded = tok(raw, add_special_tokens=True, truncation=True, max_length=max_tokens)
            token_ids = list(encoded.get("input_ids") or [])[:max_tokens]
            subtokens = [str(x) for x in tok.convert_ids_to_tokens(token_ids)]
            return {
                "ok": True,
                "schema": "SarahMemory.token_path.v0.1",
                "mode": "hf_local_tokenizer",
                "model_id": model_id or (selected_record or {}).get("id"),
                "local_dir": local_dir,
                "token_count": len(token_ids),
                "token_ids": token_ids,
                "subtokens": subtokens,
                "embedding_indices": token_ids,
        ...
        tokenizer_error = "transformers_unavailable_or_no_local_model"

    subtokens = _sm_basic_subtokens(raw)[:max_tokens]
    token_ids = [int(hashlib.sha256(s.encode("utf-8", "ignore")).hexdigest()[:8], 16) for s in subtokens]
    return {
        "ok": True,
        "schema": "SarahMemory.token_path.v0.1",
        "mode": "deterministic_fallback_subtoken_path",
        "tokenizer_error": tokenizer_error,
        "model_id": model_id or (selected_record or {}).get("id") if isinstance(selected_record, dict) else model_id,
        "local_dir": local_dir,
        "token_count": len(token_ids),
        "token_ids": token_ids,
        "subtokens": subtokens,
...
def build_sarahmemory_tokenizer_profile(
    text_samples: Optional[List[str]] = None,
    domain: str = "general",
    write: bool = True,
) -> Dict[str, Any]:
    """Build a SarahMemory-owned tokenizer profile from SEL/QIST/domain phrases.

    This does not train a foundation model. It creates the first local profile layer
    SarahMemory can use to tokenize its own governance concepts before model inference.
    """
    samples = [str(x or "") for x in (text_samples or []) if str(x or "").strip()]
    canonical_terms = [
        "SEL", "QIST", "CognitiveCompass", "PreTokenAnalyzer", "AdvCU", "LogicCalc", "Synapes", "Neuron",
        "Compare", "Roach Motel", "governed cognition", "fast answer lane", "operator lane", "model governance",
        "token path", "activation telemetry", "adapter quarantine", "local-first", "user authority", "rollback",
        "deny-by-default", "hallucination score", "drift score", "groundedness", "SarahMemory AiOS",
    ]
    term_counts: Dict[str, int] = {t: 1 for t in canonical_terms}
    for sample in samples:
        for tok in _sm_basic_subtokens(sample):
            key = str(tok).strip()
            if len(key) >= 3:
                term_counts[key] = term_counts.get(key, 0) + 1
    vocab = [k for k, _ in sorted(term_counts.items(), key=lambda kv: (-kv[1], kv[0].lower()))[:512]]
    profile = {
        "ok": True,
        "schema": _SM_TOKENIZER_PROFILE_SCHEMA,
        "domain": str(domain or "general"),
        "created_at": _now_iso(),
        "profile_id": "sarahmemory_self_tokenizer_v0_1",
        "vocab_size": len(vocab),
        "sel_vocab": vocab,
        "domain_phrases": canonical_terms,
        "compression_rules": {
            "governed_cognition": "SEL+QIST+Compass+Compare",
            "fast_answer": "answer_only,no_tool,no_mutation,no_network",
            "operator_action": "SEL_FULL,policy,simulation,verify,rollback",
            "model_forensics": "token_path,fuzz,activation,ablation,quarantine",
        },
        "qist_candidate_rules": [dict(x) for x in (
            {"candidate": "answer_only", "requires": ["low_risk", "no_mutation"]},
            {"candidate": "local_memory_read", "requires": ["read_only"]},
            {"candidate": "network_research", "requires": ["network_permission"]},
            {"candidate": "model_governance", "requires": ["tokenizer_hash", "model_hash", "user_authority"]},
            {"candidate": "security_forensics", "requires": ["roach_motel", "controlled_replay"]},
        )],
        "token_stats": {"samples": len(samples), "terms": len(term_counts)},
        "network_used": False,
        "execution_authority": False,
    }
    if write:
        outdir = _sm_tokenizer_profile_dir()
        path = os.path.join(outdir, "sarahmemory_self_tokenizer_profile_v0_1.json")
...
def get_sarahmemory_tokenizer_profile_status() -> Dict[str, Any]:
    outdir = _sm_tokenizer_profile_dir()
    profiles = []
    try:
        for name in sorted(os.listdir(outdir)):
            if name.endswith(".json"):
                ...
                profiles.append({"name": name, "path": fp, "size": os.path.getsize(fp), "sha256": _sm_sha256_file(fp)})
    except Exception:
        pass
    return {"ok": True, "schema": _SM_TOKENIZER_PROFILE_SCHEMA, "profiles_dir": outdir, "profiles": profiles, "profile_count": len(profiles)}


def bootstrap_local_sarahmemory_model(
    repo: str = "",
    allow_download: bool = False,
    build_tokenizer_profile: bool = True,
    run_smoke: bool = False,
) -> Dict[str, Any]:
    """Governed local model bootstrap.

    It selects an installed local reasoning/chat model if one exists. If no model exists,
    it returns an explicit action_required response unless allow_download=True.
    """
    scan = scan_model_registry(persist=True)
    status = get_model_manager_status(refresh=False)
    active = get_active_model_record("reasoning") or get_active_model_record("chat")
    installed = [m for m in status.get("models", []) or [] if m.get("valid")]
    selected = active if active else (installed[0] if installed else None)
    result: Dict[str, Any] = {
        "ok": True,
        "schema": _SM_BOOTSTRAP_SCHEMA,
        "scan": {"model_count": status.get("model_count"), "installed_valid": len(installed)},
        "selected_model": selected,
        "download_attempted": False,
        "network_used": False,
        "smoke_test": {"requested": bool(run_smoke), "status": "not_run"},
        "execution_authority": False,
    }
    if not selected:
        wanted = str(repo or route_to_best_available_model_repo("reasoning", "local SarahMemory reasoning model") or "").strip()
        result["recommended_repo"] = wanted
        if not allow_download:
            result.update({
                "ok": False,
                "status": "missing_local_generation_model",
                "action_required": "Install or download a small local reasoning/chat model, then rerun bootstrap.",
                "download_route": "/api/models/download with explicit user approval",
            })
        else:
            dl = download_model_to_registry(category="reasoning", repo=wanted)
            result["download_attempted"] = True
            result["network_used"] = True
            result["download_result"] = dl
            scan_model_registry(persist=True)
            selected = get_active_model_record("reasoning") or get_active_model_record("chat")
            result["selected_model"] = selected
            result["ok"] = bool(selected)
    if build_tokenizer_profile:
        result["tokenizer_profile"] = build_sarahmemory_tokenizer_profile(text_samples=[], write=True)
    if selected:
        set_active_model(str(selected.get("category") or "reasoning"), model_id=str(selected.get("id") or ""))
        result["active_model_set"] = True
        result["artifact_hashes"] = _sm_model_artifact_hashes(str(selected.get("path") or ""))
    return result


def get_model_forensics_status() -> Dict[str, Any]:
    """Report readiness of the model-forensics pipeline without running heavy probes."""
    tok_status = get_sarahmemory_tokenizer_profile_status()
    return {
        "ok": True,
        "schema": _SM_FORENSICS_SCHEMA,
        "pipeline": {
            "token_path_instrumentation": "available",
            "trigger_fuzzing_harness": "planned_bounded",
            "activation_capture": "requires_transformers_hooks_and_explicit_research_mode",
            "attention_maps": "requires_model_runtime_hooks",
            "activation_clustering": "planned_research_mode",
            "spectral_analysis": "planned_research_mode",
            "neuron_ablation": "planned_quarantine_only",
            "gradient_influence": "planned_quarantine_only",
            "serving_layer_tamper_checks": "available_artifact_hashes",
            "qist_candidate_meaning_tests": "available_via_SarahMemoryQuantumSafe",
            "sel_gating": "available_via_SarahMemoryPreTokenAnalyzer",
            "roach_motel": "required_for_security_forensics_lane",
        },
        "tokenizer_profiles": tok_status,
        "network_default": "deny",
        "shell_default": "deny",
        "filesystem_write_default": "deny",
        "execution_authority": False,
    }
