...
def _local_llm_generation_available() -> bool:
    """Metadata-only check for a governed local text-generation model.

    The local model lane must not depend only on LOCAL_LLM_API flags when the
    user has selected/installed a model through SarahMemoryLLM. This helper does
    not load a model or download anything.
    """
    try:
        if bool(getattr(config, "LOCAL_LLM_API", False) or getattr(config, "LOCAL_LLM_ENABLED", False)):
            return True
    except Exception:
        pass
    try:
        import SarahMemoryAPI as _SMAPI  # type: ignore
        fn = getattr(_SMAPI, "_provider_is_enabled", None)
        if callable(fn) and bool(fn("local_llm")):
            return True
    except Exception:
        pass
    try:
        ...
        for category in ("reasoning", "coder"):
            ...
            if isinstance(active, dict):
                local_dir = os.path.abspath(os.path.expanduser(str(active.get("local_dir") or "")))
...
class LocalResearch:
    ...
    def _search_llm_ensemble(query: str, intent: str) -> Optional[ResearchResult]:
        ...
        """
        try:
            if not _local_llm_generation_available():
                research_debug_logger.debug("[LOCAL LLM] No enabled or user-selected local text-generation model discovered; skipping local generation")
                return None
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryResearch.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryResearch', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
