...
# DL Engine ticket accountability/workorder bridge.  The UI may approve a
# sandbox/reverify/rollback decision, but this function never mutates production
# code. It creates auditable workorders and accountability reports only.
# =============================================================================
def _dl_data_dir() -> str:
    try:
        return str(getattr(config, "DATA_DIR", os.path.join(getattr(config, "BASE_DIR", os.getcwd()), "data")))
    except Exception:
        return os.path.join(os.getcwd(), "data")


def _dl_safe_name(value: Any, default: str = "ticket") -> str:
    raw = str(value or default).strip()
    raw = re.sub(r"[^A-Za-z0-9._:-]+", "_", raw).strip("._-")
    return (raw or default)[:180]


def _dl_ensure_dir(path: str) -> str:
    try:
...
def _dl_write_json(path: str, payload: Dict[str, Any]) -> None:
    _dl_ensure_dir(os.path.dirname(path))
    tmp = path + ".tmp"
...
def _dl_risk_tier(score: Any) -> str:
    try:
        n = float(score)
    except Exception:
        n = 0.0
...
def _dl_infer_affected_systems(subject: Dict[str, Any]) -> List[Dict[str, Any]]:
    raw = subject.get("affected_systems")
    if isinstance(raw, list) and raw:
        ...
        for item in raw:
            if isinstance(item, dict):
                ...
                out.append({"system_name": str(item), "impact_level": "unknown", "was_changed": False})
        return out
    blob = json.dumps(subject, ensure_ascii=False, default=str).lower()
    mapping = [
        ("chat_ingress", ("chat", "query", "rephrase", "reply")),
        ("research_lane", ("research", "websym", "knowledge", "who is", "what is")),
        ("local_database_access", ("database", "sqlite", "dataset", "qa_cache", "ai_learning")),
        ("vector_index", ("vector", "embedding", "faiss", "index")),
        ("local_llm_organs", ("llm", "model", "reasoning", "qwen", "ollama")),
        ("devbridge_ticketing", ("ticket", "devbridge", "workorder")),
        ("audit_system", ("audit", "accountability", "report")),
        ("security_governor", ("security", "firewall", "roachmotel", "agent")),
        ("energetics", ("energetics", "hazardous-energy", "hazardous energy")),
    ]
    out = []
    for name, keys in mapping:
        if any(k in blob for k in keys):
            out.append({
                "system_name": name,
                "system_type": "runtime_lane",
                "impact_type": "review",
                "impact_level": _dl_risk_tier(subject.get("risk") or subject.get("risk_score") or 0),
                "was_changed": False,
                "rollback_available": False,
            })
    return out or [{"system_name": "dl_engine_review", "system_type": "audit_console", "impact_type": "review", "impact_level": "low", "was_changed": False, "rollback_available": False}]


def record_dlengine_ticket_action(subject: Dict[str, Any], action: str = "review", *, source: str = "api") -> Dict[str, Any]:
    """Create an auditable DL Engine workorder/action record.

    Supported action meanings:
    - sandbox/test/approve/reject: operator decision record; no production apply.
    - reverify: creates a REVERIFY_TICKET workorder.
    - rollback: creates a ROLLBACK_RESTORE workorder; final user approval still required.
    - audit_only/hold: records disposition without mutation.
    """
    subject = dict(subject or {})
    action_key = str(action or subject.get("stage") or subject.get("action") or "review").lower().strip().replace(" ", "_")
    ticket_id = _dl_safe_name(subject.get("id") or subject.get("ticket_id") or subject.get("subject_id") or hashlib.sha256(json.dumps(subject, sort_keys=True, default=str).encode("utf-8")).hexdigest()[:16])
    now = datetime.now().isoformat(timespec="seconds")
    risk_score = int(max(0, min(100, float(subject.get("risk_score", subject.get("risk", 0)) or 0))))
    confidence_score = int(max(0, min(100, float(subject.get("confidence_score", subject.get("confidence", 0)) or 0))))
    affected = _dl_infer_affected_systems(subject)
    workorder_type = {
        "reverify": "REVERIFY_TICKET",
        "rollback": "ROLLBACK_RESTORE",
        "approved": "APPROVE_SANDBOX_PENDING_FINAL_APPLY",
        "approve": "APPROVE_SANDBOX_PENDING_FINAL_APPLY",
        "sandbox": "SANDBOX_TICKET",
        "testing": "TEST_TICKET",
        "test": "TEST_TICKET",
        "rejected": "REJECT_AND_ARCHIVE",
        "reject": "REJECT_AND_ARCHIVE",
        "audit_only": "AUDIT_ONLY",
        "hold": "HOLD_MANUAL_REVIEW",
    }.get(action_key, "OPERATOR_REVIEW")
    payload = {
        "schema": "SARAHMEMORY_DLENGINE_TICKET_ACTION_V1",
        "workorder_id": f"{workorder_type.lower()}_{ticket_id}_{int(time.time())}",
        "workorder_type": workorder_type,
        "source_ticket_id": ticket_id,
        "created_at": now,
        "requested_by": "USER_OR_OPERATOR_UI",
        "source": source,
        "operator_action": action_key,
        "title": str(subject.get("title") or "DL Engine subject"),
        "summary": str(subject.get("summary") or subject.get("rationale") or ""),
        "notes": str(subject.get("notes") or ""),
        "confidence_score": confidence_score,
        "risk": {
            "risk_score": risk_score,
            "risk_tier": _dl_risk_tier(risk_score),
            "risk_assessed_by": "DL Engine operator review bridge",
            "who_is_at_risk": ["user", "local system", "runtime integrity"],
            "what_is_at_risk": [a.get("system_name") for a in affected],
            "why_this_is_risky": "The ticket may affect local-first routing, memory, UI/backend bridge behavior, security posture, or rollback state. Production mutation is not authorized by this record.",
        },
        "affected_systems": affected,
        "accountability_chain": {
            "originator": subject.get("originator") or subject.get("source") or "unknown",
            "reviewer": "DL Engine UI / operator",
            "risk_assessor": "SarahMemoryDL.record_dlengine_ticket_action",
            "approver": "user required for final production apply",
            "executor": "none - workorder only",
            "verifier": "pending sandbox/reverify/rollback review",
            "user_override": False,
        ...
        "required_checks": [
            "reload_original_ticket",
            "inspect_affected_files_or_routes",
            "run_sandbox_if_safe",
            "compare_claim_vs_code_or_runtime_evidence",
            "produce_plain_english_verdict",
        ],
        "allowed_actions": ["read", "analyze", "sandbox", "prepare_restore_patch" if action_key == "rollback" else "record_decision"],
        "blocked_actions": ["auto_apply", "delete_original", "overwrite_without_backup", "persistent_policy_change"],
        "final_user_approval_required": True,
        "original_subject": subject,
    }
    data_dir = _dl_data_dir()
    repair_dir = _dl_ensure_dir(os.path.join(data_dir, "devbridge", "repair_tickets"))
    account_dir = _dl_ensure_dir(os.path.join(data_dir, "audit", "accountability"))
    report_dir = _dl_ensure_dir(os.path.join(data_dir, "audit", "reports"))
    rollback_dir = _dl_ensure_dir(os.path.join(data_dir, "audit", "rollbacks"))
    fname = _dl_safe_name(payload["workorder_id"]) + ".json"
    workorder_path = os.path.join(repair_dir, fname)
    audit_path = os.path.join(account_dir, fname)
    report_path = os.path.join(report_dir, fname)
    _dl_write_json(workorder_path, payload)
    _dl_write_json(audit_path, payload)
    _dl_write_json(report_path, payload)
    if action_key == "rollback":
        _dl_write_json(os.path.join(rollback_dir, fname), payload)
    return {"ok": True, "accepted": True, "action": action_key, "workorder": payload, "paths": {"workorder": workorder_path, "accountability": audit_path, "report": report_path}, "ts": now}
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryDL.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryDL', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
