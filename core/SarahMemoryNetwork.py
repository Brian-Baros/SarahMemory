...
# ---------------------------------------------------------------------------
# Energetics bridge: network/device power-state preflight
# ---------------------------------------------------------------------------
def _energetics_network_preflight(action_type: str, requested_power_mode: str = "ACTIVE", *, device_type: str = "network", context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Return Energetics verdict for network work. Advisory; no direct power mutation.

    Industrial safety rule: network scans, heartbeats, sync bursts, tunnels, and device-power
    modes are treated as hazardous-energy-sensitive because they may wake radios/adapters,
    increase load, create write traffic, or interfere with an embodied task. This bridge
    checks SarahMemoryGlobals before trusting Energetics and fails closed for network power work.
    """
    ctx = dict(context or {}) if isinstance(context, dict) else {}
    ctx.update({"action_type": action_type, "requested_power_mode": requested_power_mode, "device_type": device_type, "source": "SarahMemoryNetwork"})
    try:
        blocker_fn = getattr(config, "sm_hazardous_energy_blocks_action", None)
        status_fn = getattr(config, "sm_hazardous_energy_status", None)
        if callable(blocker_fn) and blocker_fn("network_device_power", ctx):
            status = status_fn(ctx) if callable(status_fn) else {}
            return {
                "ok": False,
                "decision": "DEFER",
                "reason": "Hazardous-energy constitution blocks network/device power escalation until Energetics preflight clears.",
                "allowed_power_mode": "LOW_POWER",
                "requested_power_mode": requested_power_mode,
                "constitution": status,
                "source": "SarahMemoryNetwork._energetics_network_preflight",
            }
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DEFER",
            "reason": f"Network hazardous-energy constitution check failed closed: {exc}",
            "allowed_power_mode": "LOW_POWER",
            "requested_power_mode": requested_power_mode,
            "source": "SarahMemoryNetwork._energetics_network_preflight",
        }
    try:
        ...
        fn = getattr(_Energetics, "preflight_network_action", None)
        if callable(fn):
            return fn(action_type=action_type, requested_power_mode=requested_power_mode, device_type=device_type, **ctx)
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DEFER",
            "reason": f"Energetics network bridge failed closed: {exc}",
            "allowed_power_mode": "LOW_POWER",
            "requested_power_mode": requested_power_mode,
        }
    return {"ok": False, "decision": "DEFER", "reason": "Energetics bridge unavailable; network/device power action fails closed.", "allowed_power_mode": "LOW_POWER", "requested_power_mode": requested_power_mode}


def _network_power_allowed(action_type: str, requested_power_mode: str = "ACTIVE", *, device_type: str = "network", context: Optional[Dict[str, Any]] = None) -> Tuple[bool, Dict[str, Any]]:
    verdict = _energetics_network_preflight(action_type, requested_power_mode, device_type=device_type, context=context)
    decision = str(verdict.get("decision") or "ALLOW").upper()
    allowed = decision not in {"DENY", "DEFER"}
    return allowed, verdict


def network_power_preflight(action_type: str = "network_action", requested_power_mode: str = "ACTIVE", device_type: str = "network", context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    return _energetics_network_preflight(action_type, requested_power_mode, device_type=device_type, context=context or {})


def recommend_network_device_mode(device_type: str = "network", requested_power_mode: str = "ACTIVE", context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    # Device power recommendation is itself governed. This function may suggest LOW_POWER/READY
    # only after the constitution and Energetics agree; otherwise it returns a non-authoritative defer verdict.
    verdict = network_power_preflight("device_power_state", requested_power_mode, device_type, context)
    if str(verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return verdict
    try:
        import SarahMemoryEnergetics as _Energetics  # type: ignore
        fn = getattr(_Energetics, "recommend_device_power_mode", None)
        if callable(fn):
            return fn(device_type=device_type, requested_power_mode=requested_power_mode, context=context or {})
    except Exception as exc:
        return {"ok": False, "decision": "DEFER", "allowed_power_mode": "LOW_POWER", "reason": f"Network device recommendation failed closed: {exc}"}
    return verdict
...
class NetworkNode:
    ...
    def send(self, peer_id: str, payload: bytes, meta: dict = None, flags: int = 0):
        ctx = {"peer_id": peer_id, "payload_bytes": len(payload or b""), "urgent": bool(flags & NetworkProtocol.F_URGENT)}
        allowed, verdict = _network_power_allowed("send", "ACTIVE" if not ctx["urgent"] else "FULL_POWER", context=ctx)
        if not allowed:
            self._log(f"[energetics] network send deferred/denied: {verdict.get('reason') or verdict.get('decision')}")
            return False
        meta = dict(meta or {})
        meta.setdefault("energetics_power_mode", verdict.get("allowed_power_mode", "ACTIVE"))
...
class ServerConnection:
    ...
    def start_heartbeat(self):
        ...
        allowed, verdict = _network_power_allowed("heartbeat_start", "READY", context={"scheduled": True, "heartbeat_sec": self.heartbeat_sec})
        if not allowed:
            try:
                logging.info("[NET][ENERGETICS] Heartbeat start deferred/denied: %s", verdict.get("reason") or verdict.get("decision"))
            except Exception:
                pass
            return False
    ...
    async def _hb_loop(self):
        ...
        while True:
            try:
                allowed, verdict = _network_power_allowed("heartbeat", "READY", context={"scheduled": True, "node_id": self.node_id})
                if allowed:
                    await self.register_node(self.node_id, {"ts": int(time.time()), "energetics": {"allowed_power_mode": verdict.get("allowed_power_mode"), "decision": verdict.get("decision")}})
                await asyncio.sleep(self.heartbeat_sec if allowed else max(self.heartbeat_sec, 60))
            except Exception:
    ...
    def list_bluetooth_devices(self) -> list[dict]:
        ...
        allowed, verdict = _network_power_allowed("bluetooth_scan", "FULL_POWER", device_type="bluetooth", context={"scheduled": True})
        if not allowed:
            return []
        try:
            import bluetooth  # PyBluez
            devices = bluetooth.discover_devices(duration=6, lookup_names=True)
            return [{"address": addr, "name": name or "", "energetics_power_mode": verdict.get("allowed_power_mode", "ACTIVE")} for addr, name in devices]
        except Exception:
            return []
    ...
    def wifi_scan(self) -> list[dict]:
        ...
        allowed, verdict = _network_power_allowed("wifi_scan", "FULL_POWER", device_type="wifi", context={"scheduled": True})
        if not allowed:
            return []
        ...
        try:
            if sys.platform.startswith("win"):
                ...
                return [{"ssid": s.strip(), "energetics_power_mode": verdict.get("allowed_power_mode", "ACTIVE")} for s in ssids if s.strip()]
            else:
                out = subprocess.check_output(["nmcli", "-t", "-f", "SSID", "device", "wifi", "list"], encoding="utf-8", errors="ignore")
                return [{"ssid": s.strip(), "energetics_power_mode": verdict.get("allowed_power_mode", "ACTIVE")} for s in out.splitlines() if s.strip()]
        except Exception:
            return []

    def wifi_connect(self, ssid: str, password: str | None = None) -> bool:
        """Connect to Wi-Fi SSID using OS tools (best effort)."""
        allowed, verdict = _network_power_allowed("wifi_connect", "FULL_POWER", device_type="wifi", context={"ssid": ssid, "scheduled": False})
        if not allowed:
            return False
...
class MobileIO:
    ...
    def scan_bluetooth(self) -> dict:
        allowed, verdict = _network_power_allowed("bluetooth_scan", "FULL_POWER", device_type="bluetooth", context={"scheduled": True})
        if not allowed:
            return {"ok": False, "reason": "energetics_deferred", "energetics": verdict}
        try:
            import bluetooth  # PyBluez
            devs = bluetooth.discover_devices(duration=4, lookup_names=True) or []
            return {"ok": True, "devices": devs, "energetics": {"allowed_power_mode": verdict.get("allowed_power_mode")}}
        except Exception as e:
    ...
    def scan_wifi(self) -> dict:
        ...
        try:
            ...
            return {"ok": True, "networks": nets, "energetics": {"allowed_power_mode": verdict.get("allowed_power_mode")}}
        except Exception as e:
            return {"ok": False, "reason": str(e)}
...
def heartbeat_server(base_url=None, timeout=4.0, allow_insecure_env="SARAHMEMORY_ALLOW_INSECURE_API"):
    ...
    allowed, verdict = _network_power_allowed("heartbeat_server", "READY", context={"scheduled": True})
    if not allowed:
        logging.info("[NET][ENERGETICS] Heartbeat skipped: %s", verdict.get("reason") or verdict.get("decision"))
        return False, None
    try:
        import requests
    except Exception:
...
def get_interop_broker_policy() -> Dict[str, Any]:
    return {
        ...
        "cloud_optional": True,
        "energetics_preflight_required": True,
        "network_power_states": ["OFF", "LOW_POWER", "READY", "ACTIVE", "FULL_POWER"],
        "full_power_requires_reserve_surplus": True,
    }
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryNetwork.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryNetwork', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
