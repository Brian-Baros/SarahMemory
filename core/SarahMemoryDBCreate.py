...
def _host_hints() -> Path | None:
    # SARAHMEMORY_PATCH_NOTE: host hints stay portable; no hardcoded cloud path.
    candidates = [
...
CREATE INDEX IF NOT EXISTS idx_db_registry_name ON db_registry(db_name);
CREATE INDEX IF NOT EXISTS idx_table_registry_route ON table_registry(query_types, read_allowed, priority);
CREATE INDEX IF NOT EXISTS idx_semantic_pool_routes_lookup ON semantic_pool_routes(query_type, intent, enabled, priority);
"""
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryDBCreate.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryDBCreate', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
