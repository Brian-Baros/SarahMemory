...
try:
    from SarahMemoryMusicGenerator import generate_tone, MIDINote as Note, SAMPLE_RATE as MUSIC_SAMPLE_RATE
...
def ensure_lyrics_directories() -> None:
    """Create creative-workspace directories only when the engine is explicitly initialized."""
    for directory in [LYRICS_DIR, LYRICS_PROJECTS_DIR, LYRICS_SOURCE_DIR,
        ...
        try:
            ...
            logger.warning(f"[LyricsToSong] Could not create directory {directory}: {exc}")
...
class VocalSynthesizer:
    ...
    def __init__(self):
        """Initialize the Vocal Synthesizer without unbounded model scans."""
        ensure_lyrics_directories()
        ...
        voices_dir = str(VOICE_DIR or os.path.join(BASE_DIR, "resources", "voices"))
        autoload_custom = str(os.getenv("SARAH_CREATIVE_AUTOLOAD_CUSTOM_VOICES", "false")).strip().lower() in {"1", "true", "yes", "on"}
        try:
            if SMG is not None:
                autoload_custom = bool(getattr(SMG, "CREATIVE_AUTOLOAD_CUSTOM_VOICES", autoload_custom))
        except Exception:
            pass
        max_models = max(0, min(32, int(os.getenv("SARAH_CREATIVE_MAX_CUSTOM_VOICES", "8") or 8)))

        if autoload_custom and CUSTOM_TTS_AVAILABLE and os.path.isdir(voices_dir):
            candidates = sorted(file for file in os.listdir(voices_dir) if file.lower().endswith(".pt"))[:max_models]
            for file in candidates:
                ...
                try:
                    ...
                    logger.error(f"[VoiceEngine] Failed to load custom voice {file}: {exc}")
        elif not autoload_custom:
            logger.info("[VoiceEngine] Custom voice model autoload disabled; models load only when explicitly requested.")
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        """Return a truthful, read-only vocal-engine capability report."""
        return {
            "ok": True,
            "module": "SarahMemoryLyricsToSong",
            "version": LYRICS_TO_SONG_VERSION,
            "build": LYRICS_TO_SONG_BUILD,
            "sample_rate": int(self.sample_rate),
            "supported_audio_formats": list(SUPPORTED_AUDIO_FORMATS),
            "tts_engines": {name: bool(value) for name, value in TTS_ENGINES_AVAILABLE.items()},
            "audio_processing_available": bool(AUDIO_PROCESSING_AVAILABLE),
            "scipy_available": bool(SCIPY_AVAILABLE),
            "custom_tts_available": bool(CUSTOM_TTS_AVAILABLE),
            "custom_models_loaded": len(self.custom_voice_models),
            "local_only_mode": bool(LOCAL_ONLY_MODE),
            "safe_mode": bool(SAFE_MODE),
            "execution_authority": False,
        }

    def build_output_manifest(self, project: VocalProject, output_path: str = "") -> Dict[str, Any]:
        """Build a bounded vocal-output manifest without rendering audio."""
        return {
            "schema": "SARAHMEMORY_VOCAL_OUTPUT_V1",
            "project_id": getattr(project, "project_id", ""),
            "name": getattr(project, "name", ""),
            "tempo": float(getattr(project, "tempo", 0.0) or 0.0),
            "sample_rate": int(self.sample_rate),
            "track_count": len(getattr(project, "vocal_tracks", []) or []),
    ...
    def enterprise_self_test(self) -> Dict[str, Any]:
        """Run bounded in-memory lyric/audio invariants; no TTS call and no file write."""
        checks: List[Dict[str, Any]] = []
        lines = parse_lyrics_to_lines("Verse one\n\nChorus line")
        checks.append({"name": "lyric_parse", "passed": len(lines) == 2, "observed": lines})
        syllables = syllable_count("SarahMemory")
        checks.append({"name": "syllable_count", "passed": isinstance(syllables, int) and syllables >= 1, "observed": syllables})
        frequency = note_to_frequency("A", 4)
        checks.append({"name": "a4_frequency", "passed": abs(frequency - 440.0) < 1e-6, "observed": frequency})
        test_audio = np.array([0.0, 0.25, -0.25], dtype=np.float32)
        normalized = normalize_audio(test_audio, target_db=-20.0)
        checks.append({"name": "audio_finite", "passed": bool(np.all(np.isfinite(normalized))), "observed": normalized.tolist()})
...
def get_lyrics_to_song_capabilities() -> Dict[str, Any]:
    """Read-only module capability report without initializing TTS engines."""
    return {
        "ok": True,
        "module": "SarahMemoryLyricsToSong",
        "version": LYRICS_TO_SONG_VERSION,
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryLyricsToSong.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryLyricsToSong',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryLyricsToSong', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
