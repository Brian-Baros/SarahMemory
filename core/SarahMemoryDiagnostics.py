...
# BASE_DIR is the project root; SarahMemory*.py organs live under BASE_DIR/core.
# This fixes false missing-file errors when launching from root, core, a shortcut, or another drive letter.
_CORE_DIR = getattr(config, "CORE_DIR", os.path.join(getattr(config, "BASE_DIR", os.getcwd()), "core"))
def _core_file(name: str) -> str:
    return os.path.join(_CORE_DIR, name)

def _api_file(name: str) -> str:
    return os.path.join(config.API_DIR, "server", name)

REQUIRED_FILES = [
    _api_file("app.py"),
    _api_file("appnet.py"),
    _api_file("appnet2.py"),
    _api_file("appsys.py"),
    _api_file("appdrivers.py"),
    _api_file("appcomm.py"),
    _api_file("appmedia.py"),
    _core_file("SarahMemoryADDONLCHR.py"),
    _core_file("SarahMemoryAPI.py"),
    _core_file("SarahMemoryARILE.py"),
    _core_file("SarahMemoryAdaptive.py"),
    _core_file("SarahMemoryAdvCU.py"),
    _core_file("SarahMemoryAiFunctions.py"),
    _core_file("SarahMemoryAssuranceGate.py"),
    _core_file("SarahMemoryAudit.py"),
    _core_file("SarahMemoryAvatar.py"),
    _core_file("SarahMemoryAvatarPanel.py"),
    _core_file("SarahMemoryBrowser.py"),
    _core_file("SarahMemoryCanvasStudio.py"),
    _core_file("SarahMemoryCleanup.py"),
    _core_file("SarahMemoryCleanupDaily.py"),
    _core_file("SarahMemoryCognitiveCompass.py"),
    _core_file("SarahMemoryCognitiveIdentityLayer.py"),
    _core_file("SarahMemoryCognitiveSelf.py"),
    _core_file("SarahMemoryCognitiveServices.py"),
    _core_file("SarahMemoryCognitiveThinker.py"),
    _core_file("SarahMemoryCompare.py"),
    _core_file("SarahMemoryCryptoGenesis.py"),
    _core_file("SarahMemoryDBCreate.py"),
    _core_file("SarahMemoryDL.py"),
    _core_file("SarahMemoryDataAuditor.py"),
    _core_file("SarahMemoryDatabase.py"),
    _core_file("SarahMemoryDesktop.py"),
    _core_file("SarahMemoryDiagnostics.py"),
    _core_file("SarahMemoryEmail.py"),
    _core_file("SarahMemoryEncryption.py"),
    _core_file("SarahMemoryEnergetics.py"),
    _core_file("SarahMemoryEvolution.py"),
    _core_file("SarahMemoryExpressOut.py"),
    _core_file("SarahMemoryFacialRecognition.py"),
    _core_file("SarahMemoryFilesystem.py"),
    _core_file("SarahMemoryGITtalk.py"),
    _core_file("SarahMemoryGUI.py"),
    _core_file("SarahMemoryGUI2.py"),
    _core_file("SarahMemoryGlobals.py"),
    _core_file("SarahMemoryHi.py"),
    _core_file("SarahMemoryInitialization.py"),
    _core_file("SarahMemoryIntegration.py"),
    _core_file("SarahMemoryLLM.py"),
    _core_file("SarahMemoryLedger.py"),
    _core_file("SarahMemoryLogicCalc.py"),
    _core_file("SarahMemoryLyricsToSong.py"),
    _core_file("SarahMemoryMSDC.py"),
    _core_file("SarahMemoryMain.py"),
    _core_file("SarahMemoryMigrations.py"),
    _core_file("SarahMemoryMusicGenerator.py"),
    _core_file("SarahMemoryNetwork.py"),
    _core_file("SarahMemoryNeuron.py"),
    _core_file("SarahMemoryOperatorCore.py"),
    _core_file("SarahMemoryOptimization.py"),
    _core_file("SarahMemoryPersonality.py"),
    _core_file("SarahMemoryPreTokenAnalyzer.py"),
    _core_file("SarahMemoryQuantumSafe.py"),
    _core_file("SarahMemoryRankingupdater.py"),
    _core_file("SarahMemoryReminder.py"),
    _core_file("SarahMemoryReply.py"),
    _core_file("SarahMemoryResearch.py"),
    _core_file("SarahMemoryRhythmCognition.py"),
    _core_file("SarahMemorySMAPI.py"),
    _core_file("SarahMemorySOBJE.py"),
    _core_file("SarahMemorySafetyPolicies.py"),
    _core_file("SarahMemorySecurityGovernor.py"),
    ...
    _core_file("SarahMemorySystemLearn.py"),
    _core_file("SarahMemoryTerminal.py"),
    _core_file("SarahMemoryTree.py"),
    _core_file("SarahMemoryTrustRegistry.py"),
    _core_file("SarahMemoryUISelfAware.py"),
    _core_file("SarahMemoryUIupdater.py"),
    _core_file("SarahMemoryUpdater.py"),
    _core_file("SarahMemoryVRHudRenderer.py"),
    _core_file("SarahMemoryVault.py"),
    _core_file("SarahMemoryVideoEditorCore.py"),
    _core_file("SarahMemoryVoice.py"),
    _core_file("SarahMemoryWebSYM.py"),
    _core_file("SarahNetMCP_Diagnostics.py"),
    _core_file("UnifiedAvatarController.py"),
]
...
def get_rhythm_cognition_health() -> dict:
    ...
    try:
        ...
        if callable(fn2):
            pkt = fn2({"caller": "SarahMemoryDiagnostics.get_rhythm_cognition_health"}, force_refresh=False)
            report["available"] = True
            report["status"] = "ok"
...
def _ensure_canvas_dirs():
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"[v9.0][DIAG] Canvas dirs check failed (non-critical): {e}")
        return False
...
# ---------------------------------------------------------------------------
# Energetics preflight bridge (v9.0.0 survival/metabolic organ)
# ---------------------------------------------------------------------------
def _energetics_phase_preflight(phase: str, requested_power_mode: str = "ACTIVE", *, active_task: dict | None = None) -> dict:
    """Ask SarahMemoryEnergetics whether a diagnostics phase may run.

    Diagnostics remains mostly observational, but full/network/sync/hardware phases can wake
    devices, consume bandwidth, poll sensors, and write logs. Scheduled phases
    are proposals, not commands. The hazardous-energy constitution is checked
    before Energetics is trusted; missing/failed Energetics is not allowed to
    fail-open for high-risk diagnostics.
    """
    phase_key = str(phase or "diagnostics").strip().lower()
    context = {"phase": phase_key, "active_task": active_task or {}, "source": "SarahMemoryDiagnostics"}

    # Constitution-first gate. Full/hardware/network/sync diagnostics are hazardous-energy-sensitive because
    # they may wake devices, poll buses, write logs, or touch network state. Lightweight self_check is allowed
    # to proceed in reduced mode so the operator can see why the system is locked down.
    try:
        if phase_key not in {"self_check", "lightweight", "lightweight_self_check"}:
            blocker_fn = getattr(config, "sm_hazardous_energy_blocks_action", None)
            status_fn = getattr(config, "sm_hazardous_energy_status", None)
            if callable(blocker_fn) and blocker_fn("diagnostics_full", context):
                status = status_fn(context) if callable(status_fn) else {}
                return {
                    "ok": False,
                    "decision": "DEFER",
                    "reason": "Hazardous-energy constitution restricts diagnostics to lightweight/read-only mode until Energetics preflight clears.",
                    "allowed_power_mode": "LOW_POWER",
                    "diagnostics_mode": "LIGHTWEIGHT_ONLY",
                    "constitution": status,
                    "source": "SarahMemoryDiagnostics._energetics_phase_preflight",
                }
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DEFER",
            "reason": f"Diagnostics hazardous-energy constitution check failed closed: {exc}",
            "allowed_power_mode": "LOW_POWER",
            "diagnostics_mode": "LIGHTWEIGHT_ONLY",
            "source": "SarahMemoryDiagnostics._energetics_phase_preflight",
        }

    # Lightweight self-check is allowed as a read-only witness during lockout. It must not escalate into
    # hardware/network/sync/full diagnostics, but it can report that the constitution is locked down.
    if phase_key in {"self_check", "lightweight", "lightweight_self_check"}:
        try:
            status_fn = getattr(config, "sm_hazardous_energy_status", None)
            status = status_fn(context) if callable(status_fn) else {}
            if isinstance(status, dict) and bool(status.get("lockout_active", True)):
                return {
                    "ok": True,
                    "decision": "REDUCE_MODE",
                    "reason": "Hazardous-energy constitution permits lightweight/read-only self-check only.",
                    "allowed_power_mode": "LOW_POWER",
                    "diagnostics_mode": "LIGHTWEIGHT_ONLY",
                    "constitution": status,
                    "source": "SarahMemoryDiagnostics._energetics_phase_preflight",
    ...
    try:
        ...
        fn = getattr(_Energetics, "preflight_diagnostics_phase", None)
        if callable(fn):
            return fn(phase=phase, requested_power_mode=requested_power_mode, active_task=active_task or {}, context=context)
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DEFER",
            "reason": f"Energetics diagnostics bridge failed closed: {exc}",
            "allowed_power_mode": "LOW_POWER",
            "diagnostics_mode": "LIGHTWEIGHT_ONLY",
            "source": "SarahMemoryDiagnostics._energetics_phase_preflight",
        }
    return {
        "ok": False,
        "decision": "DEFER",
        "reason": "Energetics diagnostics bridge unavailable; hazardous-energy policy fails closed for non-lightweight diagnostics.",
        "allowed_power_mode": "LOW_POWER",
        "diagnostics_mode": "LIGHTWEIGHT_ONLY",
        "source": "SarahMemoryDiagnostics._energetics_phase_preflight",
    }
...
def _energetics_deferred_report(phase: str, verdict: dict) -> dict:
    return {
        "ok": False,
        "deferred": True,
        "phase": phase,
        "reason": str(verdict.get("reason") or "energetics_deferred"),
        "energetics": verdict,
        "checks": [{
            "component": "energetics",
            "kind": phase,
            "status": "DEFER",
            "detail": str(verdict.get("reason") or verdict.get("decision") or "deferred"),
        }],
    }
...
def run_self_check():
    ...
    """
    _energy_verdict = _energetics_phase_preflight("self_check", "ACTIVE")
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return _energetics_deferred_report("self_check", _energy_verdict)
...
def run_hardware_diagnostics() -> dict:
    ...
    """
    _energy_verdict = _energetics_phase_preflight("hardware_diagnostics", "ACTIVE")
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return _energetics_deferred_report("hardware_diagnostics", _energy_verdict)
    report: dict = {"ok": False, "checks": []}
...
def run_network_diagnostics() -> dict:
    ...
    """
    _energy_verdict = _energetics_phase_preflight("network_diagnostics", "ACTIVE")
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return _energetics_deferred_report("network_diagnostics", _energy_verdict)
    report: dict = {"ok": False, "checks": []}
...
def run_sync_diagnostics() -> dict:
    ...
    """
    _energy_verdict = _energetics_phase_preflight("sync_diagnostics", "FULL_POWER")
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return _energetics_deferred_report("sync_diagnostics", _energy_verdict)
    report: dict = {"ok": False, "checks": []}
...
def run_full_diagnostics_suite(write_aggregate_log: bool = False) -> dict:
    ...
    """
    _energy_verdict = _energetics_phase_preflight("full_diagnostics_suite", "FULL_POWER")
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER"}:
        return _energetics_deferred_report("full_diagnostics_suite", _energy_verdict)
...
def run_sarahnet_api_self_repair() -> dict:
    ...
    try:
        meta_db = Path(getattr(config, "META_DB_PATH", data_dir / "memory" / "datasets" / "meta.db"))
        if meta_db.exists():
...
# ---------------------------------------------------------------------------
# Semantic Memory Hygiene Diagnostics - v9.0.0
# ---------------------------------------------------------------------------
def run_database_poison_diagnostics(max_rows: int = 5000) -> dict:
    """Read-only QA cache poison report.

    This does not delete, quarantine, vacuum, reindex, or write. It is intended
    to tell the operator whether fast-answer cache rows contain policy/failure
    text, paper blobs, or wrong-topic answers before cleanup is approved.
    """
    report = {
        "ok": True,
        "mode": "read_only",
        "component": "semantic_memory_hygiene",
        "checked_rows": 0,
        "poison_count": 0,
        "poisoned": [],
        "errors": [],
    ...
    try:
        ...
        scan = _SMCleanup.scan_poisoned_qa_cache(max_rows=max_rows)
        if isinstance(scan, dict):
            report.update({
                "checked_rows": scan.get("checked_rows", 0),
                "poison_count": scan.get("poison_count", 0),
                "poisoned": scan.get("poisoned", []),
                "errors": scan.get("errors", []),
                "source": "SarahMemoryCleanup.scan_poisoned_qa_cache",
            })
            return report
        report["ok"] = False
        report["errors"].append("scan_poisoned_qa_cache returned non-dict result")
    except Exception as exc:
        report["ok"] = False
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryDiagnostics.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryDiagnostics',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryDiagnostics', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
# --- SML ORGAN ADAPTER END ---

# --- SML DIAGNOSTICS SPECIALIZATION START ---
def sml_protocol_diagnostics(core_path=None):
    """Run SML protocol diagnostics and optional bounded organ discovery."""
    from SarahMemorySMLProtocol import get_protocol
    proto = get_protocol()
    if core_path:
