...
def _build_governed_object_packet(findings: Dict[str, Any]) -> Dict[str, Any]:
    """Build a stable SOBJE object packet for AvatarPanel/3D scene use.

    This packet is observation-only. It does not persist object memory and does
    not authorize scene permanence, robotics, driver actions, or identity learning.
    """
    detections = findings.get("detections") if isinstance(findings.get("detections"), list) else []
    primary = detections[0] if detections and isinstance(detections[0], dict) else {}
    obj_name = primary.get("label") or findings.get("resolved_subject") or None
    confidence = float(findings.get("confidence") or primary.get("confidence") or 0.0)
    bbox = primary.get("bbox") if isinstance(primary.get("bbox"), (list, tuple)) else None
    object_type = str(primary.get("domain") or primary.get("type") or "visual_observation")
    packet = {
        "schema": "SarahMemory.SOBJE.object_packet.v1",
        "ok": bool(obj_name),
        "object_id": "sobje_observation_" + _norm_token(str(obj_name or "unknown"))[:40] if obj_name else None,
        ...
        "scene_proxy": {
            "enabled": bool(obj_name),
            "proxy_id": "proxy_" + _norm_token(str(obj_name or "unknown"))[:40] if obj_name else None,
            "placement_status": "estimated_2d_only",
            "scene_position": None,
            "requires_3d_scene_graph": True,
        },
        "memory_match_status": "not_checked",
        "user_verified_status": False,
        "retention_policy": "volatile_observation_only",
        "governance_state": {
            "observation_only": True,
            "persistent_learning_allowed": False,
            "identity_learning_allowed": False,
            "requires_user_approval_for_learning": True,
            "physical_action_allowed": False,
            "msdc_dispatch_allowed": False,
        },
        "allowed_actions": ["display", "describe", "ask_user_to_verify"],
    }
    return packet
...
def answer_visual_question(question, frame):
    ...
    object_packet = _build_governed_object_packet(findings)
    details = {
        "request": findings.get("vision_request") or {},
        "object_packet": object_packet,
        "governance_state": object_packet.get("governance_state") or {},
    ...
    return {"answer": answer, "details": details, "object_packet": object_packet}
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemorySOBJE.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemorySOBJE', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
