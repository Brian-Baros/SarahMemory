...
# SARAHMEMORY_PATCH_NOTE: v9 local-first online arming overlay.
# Default is now LOCAL_ONLY=True because SarahMemory must boot and reason offline first.
# The UI/session layer may temporarily arm online research/API use by changing this flag or
# by using a governed online session token; cloud/provider access is not assumed at boot.
LOCAL_ONLY_MODE = _env_flag("SARAH_LOCAL_ONLY_MODE", "true")  # True = bypass external network research/API use unless the UI/user explicitly arms it.

# SARAHMEMORY_PATCH_NOTE 2026-06-23:
# Required boot-governance overlay. These flags do not replace the existing
# governance doctrine, NEOSKYMATRIX, DEVELOPERSMODE, SMGET, Compare, Compass,
# SafetyPolicies, SecurityGovernor, AssuranceGate, OperatorCore, or Energetics.
...
SARAHMEMORY_ONLINE_SESSION_ARMED = _env_flag("SARAHMEMORY_ONLINE_SESSION_ARMED", "false")
...
ENABLE_MODEL_C  = True # sentence-transformers/multi-qa-MiniLM-L6-cos-v1 Tier: LOW | Function: QA-optimized embeddings (question->passage retrieval)
...
ENABLE_MODEL_Q  = True  # Qwen/Qwen2.5-7B-Instruct  Tier: BEAST | Function: Strong general instruct LLM; higher quality, higher resource demands
...
ENABLE_MODEL_T  = True  # Qwen/Qwen2.5-Coder-7B-Instruct   Tier: BEAST | Function: Code assistant (high quality); needs more VRAM/RAM
# -----------------------------------------------------------------------------
...
ENABLE_MODEL_W  = True  # ultralytics/yolov8 Tier: MID | Function: Stable YOLOv8 baseline; good compatibility fallback
...
ENABLE_MODEL_AA  = True  # Freepik/flux.1-lite-8B Tier: HIGH | Function: Higher quality local image generation; heavier footprint
ENABLE_MODEL_AB  = True  # black-forest-labs/FLUX.1-dev Tier: BEAST | Function: Highest tier FLUX; heavy VRAM/RAM; consider API fallback if unstable locally
# -----------------------------------------------------------------------------
...
def _sm_project_root_markers(path_value: str) -> bool:
    """Return True when path_value looks like a SarahMemory install root.

    Accepts either the v9 root/core layout or the earlier flat core layout.  This
    keeps BASE_DIR portable when the project is installed at C:/SarahMemory,
    D:/SarahMemory, S:/SarahMemory, F:/SMv9, or any user-selected folder.
    """
    try:
        p = os.path.abspath(os.path.expanduser(str(path_value or "")))
        if not os.path.isdir(p):
            return False
        return bool(
            os.path.isfile(os.path.join(p, "core", "SarahMemoryGlobals.py"))
            or os.path.isfile(os.path.join(p, "SarahMemoryGlobals.py"))
            or os.path.isfile(os.path.join(p, "core", "SarahMemoryMain.py"))
            or os.path.isfile(os.path.join(p, "SarahMemoryMain.py"))
        )
    except Exception:
        return False


def _sm_normalize_project_root(candidate: str) -> str | None:
    """Normalize env/cwd/file paths to the SarahMemory install root.

    If a launcher, bridge, or operator points BASE_DIR/SARAHMEMORY_ROOT at
    ./core, ./api, or ./api/server, return the owning install root instead.
    """
    try:
        p = os.path.abspath(os.path.expanduser(str(candidate or "")))
        if not os.path.isdir(p):
            return None
        search = [p]
        parent = os.path.abspath(os.path.join(p, ".."))
        grandparent = os.path.abspath(os.path.join(p, "..", ".."))
        for extra in (parent, grandparent):
            if extra not in search:
                search.append(extra)
        for item in search:
            if _sm_project_root_markers(item):
                # If item is itself ./core in a flat extracted core zip, its parent
                # should only be returned when it owns a core directory. Otherwise
                # item is the flat root.
                base_name = os.path.basename(item).lower()
                if base_name == "core" and _sm_project_root_markers(os.path.join(item, "..")):
                    return os.path.abspath(os.path.join(item, ".."))
...
def _resolve_sarahmemory_base_dir() -> str:
    """Resolve the SarahMemory project root for portable v9 operation."""
    for env_key in ("SARAHMEMORY_ROOT", "SARAH_BASE_DIR", "SARAHMEMORY_BASE_DIR", "BASE_DIR"):
        value = os.getenv(env_key)
        if value:
            resolved = _sm_normalize_project_root(value)
            if resolved:
                return resolved
    try:
        this_dir = os.path.dirname(os.path.abspath(__file__))
        resolved = _sm_normalize_project_root(this_dir)
        if resolved:
            return resolved
    except Exception:
        pass
    try:
        resolved = _sm_normalize_project_root(os.getcwd())
        if resolved:
            return resolved
    except Exception:
        pass
...
ROOT_DIR = BASE_DIR
# SARAHMEMORY_PATCH_NOTE: v9 root/core portability anchor.
# BASE_DIR is the install root. CORE_DIR is where the governed core organs live.
# Modules must use this instead of assuming SarahMemory*.py files live in BASE_DIR.
_CORE_DIR_CANDIDATE = os.path.join(BASE_DIR, "core")
CORE_DIR = _CORE_DIR_CANDIDATE if os.path.isdir(_CORE_DIR_CANDIDATE) else BASE_DIR
API_SERVER_DIR = os.path.join(BASE_DIR, "api", "server")
if not os.path.isdir(API_SERVER_DIR):
    _API_SERVER_FALLBACK = os.path.join(BASE_DIR, "api")
    API_SERVER_DIR = _API_SERVER_FALLBACK if os.path.isdir(_API_SERVER_FALLBACK) else BASE_DIR
for _sm_import_path in (CORE_DIR, API_SERVER_DIR, BASE_DIR):
    try:
        if _sm_import_path and os.path.isdir(_sm_import_path) and _sm_import_path not in sys.path:
            sys.path.insert(0, _sm_import_path)
    except Exception:
        pass
...
logger.info(f"CORE_DIR set to: {CORE_DIR}")
#
# --- UI Configuration ---
...
# =============================================================================
# These files LIVE in: ./data/ui for the v9 root/core layout
...
# V9 UI governance path correction. The actual V9 UI contract places the
# built UI at BASE_DIR/data/ui/v9 and the source at BASE_DIR/data/ui/V9_ui_src.
# This must remain rooted under BASE_DIR/data/ui so the backend, pywebview, and
# UI updater all agree on one frontend location and do not open a blank webview.
...
# Safety: If folder/index missing, auto-fallback to classic
if USE_LEGACY_WEBUI and (not os.path.isdir(LEGACY_WEBUI_DIR) or not os.path.isfile(LEGACY_WEBUI_INDEX)):
    print("[UI WARNING] Legacy WebUI directory/index missing — falling back to classic mode.")
...
# =============================================================================
# Our pipeline:
#   - SarahMemoryUIupdater.py works from:   BASE_DIR/data/ui/V9_ui_src
#   - It builds Vite "dist" there
#   - Then copies dist/* into:              BASE_DIR/data/ui/v9
#
# So the *served* dist folder is always: BASE_DIR/data/ui/v9
# =============================================================================

CUSTOM_UI_ROOT = os.getenv(
    "SARAH_CUSTOM_UI_ROOT",
    os.path.join(BASE_DIR, "data", "ui", "V9_ui_src"),
)

CUSTOM_UI_DIST_DIR = os.getenv(
    "SARAH_CUSTOM_UI_DIST_DIR",
    os.path.join(BASE_DIR, "data", "ui", "v9"),
)
...
# Safety: If custom selected but dist/index missing, auto-fallback deterministically.
if USE_CUSTOM_WEBUI and (not os.path.isdir(CUSTOM_UI_DIST_DIR) or not os.path.isfile(CUSTOM_UI_INDEX)):
    if os.path.isfile(LEGACY_WEBUI_INDEX):
        print("[UI WARNING] Custom UI dist/index missing — falling back to legacy web.")
        USE_CUSTOM_WEBUI = False
        USE_LEGACY_WEBUI = True
        USE_CLASSIC_GUI = False
        UI_MODE = "web"
    else:
        print("[UI WARNING] Custom UI and legacy WebUI missing — falling back to classic mode.")
        USE_CUSTOM_WEBUI = False
        USE_LEGACY_WEBUI = False
        USE_CLASSIC_GUI = True
        UI_MODE = "classic"
...
def get_ui_launch_profile() -> dict:
    ...
    if globals().get("USE_CLASSIC_GUI"):
        mode = "classic"
    elif globals().get("USE_CUSTOM_WEBUI"):
        mode = "custom"
    elif globals().get("USE_LEGACY_WEBUI"):
        mode = "web"
    base = {
        "mode": mode,
...
LOCAL_DATASET_POOL_MAX_HITS = int(os.getenv("SARAH_LOCAL_DATASET_POOL_MAX_HITS", "5") or 5)
LOCAL_DATASET_POOL_TIME_BUDGET_SEC = float(os.getenv("SARAH_LOCAL_DATASET_POOL_TIME_BUDGET_SEC", "1.25") or 1.25)
LOCAL_DATASET_POOL_MAX_DBS = int(os.getenv("SARAH_LOCAL_DATASET_POOL_MAX_DBS", "12") or 12)
LOCAL_DATASET_POOL_MAX_TABLES_PER_DB = int(os.getenv("SARAH_LOCAL_DATASET_POOL_MAX_TABLES_PER_DB", "20") or 20)
LOCAL_DATASET_POOL_MAX_ROWS_PER_TABLE = int(os.getenv("SARAH_LOCAL_DATASET_POOL_MAX_ROWS_PER_TABLE", "4") or 4)
...
# SARAHMEMORY_PATCH_NOTE: third-party AI/API research is disabled by default; local models and DB routes remain primary.
API_RESEARCH_ENABLED = _env_flag("SARAH_API_RESEARCH_ENABLED", "false") # True = allow governed third-party API research when user/session permits it.
...
# SARAHMEMORY_PATCH_NOTE: remote sync is disabled by default to prevent outside sources or AI agents from triggering code/data movement.
# To enable remote sync, the UI/operator must set SARAH_REMOTE_SYNC_ENABLED=true and provide a governed session token.
REMOTE_SYNC_ENABLED = _env_flag("SARAH_REMOTE_SYNC_ENABLED", "false")
# SARAHMEMORY_PATCH_NOTE: online session arming is separate from feature flags; both must pass before network actions proceed.
ONLINE_SESSION_ARMED = _env_flag("SARAH_ONLINE_SESSION_ARMED", "false")
ONLINE_SESSION_TOKEN = os.getenv("SARAH_ONLINE_SESSION_TOKEN", "").strip()
ONLINE_SESSION_REQUIRED = _env_flag("SARAH_ONLINE_SESSION_REQUIRED", "true")
ANTI_AGENT_HIJACK_ENABLED = _env_flag("SARAH_ANTI_AGENT_HIJACK_ENABLED", "true")
AUDIT_EVERY_PROCESS = _env_flag("SARAH_AUDIT_EVERY_PROCESS", "true")
...
# Creative-runtime anti-thrash policy. Heavy models/renders stay opt-in.
CREATIVE_MAX_THREADS = max(1, min(8, int(os.getenv("SARAH_CREATIVE_MAX_THREADS", "4") or 4)))
CREATIVE_AUTOLOAD_CUSTOM_VOICES = _env_flag("SARAH_CREATIVE_AUTOLOAD_CUSTOM_VOICES", "false")
CREATIVE_MAX_CUSTOM_VOICES = max(0, min(32, int(os.getenv("SARAH_CREATIVE_MAX_CUSTOM_VOICES", "8") or 8)))
BLENDER_BUILD_TIMEOUT_SECONDS = max(60, int(os.getenv("SARAH_BLENDER_BUILD_TIMEOUT_SECONDS", "7200") or 7200))
BLENDER_HEAVY_BUILD_REQUIRES_APPROVAL = _env_flag("SARAH_BLENDER_HEAVY_BUILD_REQUIRES_APPROVAL", "true")
...
# =============================================================================
# ENTERPRISE RUNTIME STATE / DATABASE PLACEMENT CONTRACT
# -----------------------------------------------------------------------------
# Only process PID markers belong directly under DATA_DIR. Persistent SQLite
# databases belong under DATASETS_DIR and mutable JSON runtime state belongs
# under SETTINGS_DIR. These canonical paths are consumed by Main, API bridges,
# diagnostics, migrations, Terminal, and the governed Ledger/Passport lanes.
# =============================================================================
SERVER_STATE_PATH       = os.path.join(SETTINGS_DIR, "server_state.json")
BROWSER_STATE_PATH      = os.path.join(SETTINGS_DIR, "browser_state.json")
META_DB_PATH            = os.path.join(DATASETS_DIR, "meta.db")
USER_DATA_DB_PATH       = os.path.join(DATASETS_DIR, "user_data.db")
CONTEXT_HISTORY_DB_PATH = os.path.join(DATASETS_DIR, "context_history.db")
REMINDERS_DB_PATH       = os.path.join(DATASETS_DIR, "reminders.db")
WALLETS_DB_PATH         = os.path.join(DATASETS_DIR, "wallets.db")
LEDGER_DB_PATH          = os.path.join(DATASETS_DIR, "SarahMemoryLedger.db")
TRUST_REGISTRY_DB_PATH  = os.path.join(DATASETS_DIR, "trust_registry.db")

# Smooth-boot / smooth-shutdown / anti-thrash policy. These are conservative
# defaults for the current FX-8350/32-GB/RTX-3060 development body and remain
# operator-overridable through environment variables.
BOOT_MEDIA_CAPABILITY_PROBE_ONLY = _env_flag("SARAH_BOOT_MEDIA_CAPABILITY_PROBE_ONLY", "true")
BOOT_RUN_MIGRATIONS_ON_STARTUP = _env_flag("SARAH_BOOT_RUN_MIGRATIONS_ON_STARTUP", "false")
BOOT_MIGRATION_BACKUP_ONLY_WHEN_PENDING = _env_flag("SARAH_BOOT_MIGRATION_BACKUP_ONLY_WHEN_PENDING", "true")
BOOT_BACKGROUND_TASK_MAX = max(1, int(os.getenv("SARAH_BOOT_BACKGROUND_TASK_MAX", "2") or 2))
BOOT_RESOURCE_CPU_HOLD_PERCENT = float(os.getenv("SARAH_BOOT_RESOURCE_CPU_HOLD_PERCENT", "92") or 92)
BOOT_RESOURCE_MEMORY_HOLD_PERCENT = float(os.getenv("SARAH_BOOT_RESOURCE_MEMORY_HOLD_PERCENT", "90") or 90)
SARAH_FORCE_PROCESS_EXIT_ON_SHUTDOWN = _env_flag("SARAH_FORCE_PROCESS_EXIT_ON_SHUTDOWN", "false")
SARAH_ALLOW_HARD_PROCESS_KILL_FALLBACK = _env_flag("SARAH_ALLOW_HARD_PROCESS_KILL_FALLBACK", "false")
SARAH_SHUTDOWN_GRACE_SECONDS = float(os.getenv("SARAH_SHUTDOWN_GRACE_SECONDS", "8") or 8)
SHUTDOWN_STATE_POLL_SECONDS = float(os.getenv("SARAH_SHUTDOWN_STATE_POLL_SECONDS", "1.5") or 1.5)
SHUTDOWN_TTS_GRACE_SECONDS = float(os.getenv("SARAH_SHUTDOWN_TTS_GRACE_SECONDS", "0.25") or 0.25)

# Governed Ledger / AI-agent passport policy. A passport identifies and scopes an
# agent; it never grants execution authority. Returning agents always enter the
# AgentFirewall/RoachMotel review lane before any sanitized evidence may reach
# Compass or Neuron.
SARAH_LEDGER_ENABLED = _env_flag("SARAH_LEDGER_ENABLED", "true")
SARAH_LEDGER_RECEIPTS_ENABLED = _env_flag("SARAH_LEDGER_RECEIPTS_ENABLED", "true")
SARAH_LEDGER_RECEIPT_MAX_METADATA_CHARS = int(os.getenv("SARAH_LEDGER_RECEIPT_MAX_METADATA_CHARS", "16000") or 16000)
SARAH_LEDGER_RETENTION_BATCH = int(os.getenv("SARAH_LEDGER_RETENTION_BATCH", "250") or 250)
SARAH_AGENT_PASSPORTS_ENABLED = _env_flag("SARAH_AGENT_PASSPORTS_ENABLED", "true")
SARAH_AGENT_PASSPORT_DEFAULT_TTL_SECONDS = int(os.getenv("SARAH_AGENT_PASSPORT_DEFAULT_TTL_SECONDS", "3600") or 3600)
SARAH_AGENT_PASSPORT_MAX_TTL_SECONDS = int(os.getenv("SARAH_AGENT_PASSPORT_MAX_TTL_SECONDS", "86400") or 86400)
SARAH_AGENT_RETURN_ALWAYS_REQUIRES_REVIEW = _env_flag("SARAH_AGENT_RETURN_ALWAYS_REQUIRES_REVIEW", "true")
SARAH_AGENT_PASSPORT_ONE_TIME_DEFAULT = _env_flag("SARAH_AGENT_PASSPORT_ONE_TIME_DEFAULT", "true")

SARAH_AGENT_PASSPORT_ID = True
# SARAH_AGENT_PASSPORT_ID = _env_flag("SARAH_AGENT_PASSPORT_ID", "True") # True for Auto Issue, False for Manual issuing.
SARAH_ASSURANCE_ENABLED = True
SARAH_ASSURANCE_TESTS_ENABLED = True
SARAH_AGENT_SWARM_TEST_ENABLED = False
SARAH_SECRET_SCAN_ENABLED = True
SARAH_STATIC_SCAN_ENABLED = True
SARAH_SBOM_ENABLED = True
SARAH_RELEASE_HASH_MANIFEST_ENABLED = True
SARAH_TRUST_TRANSITION_AUDIT_ENABLED = True
SARAH_SECURITY_REPORTS_ENABLED = True
SARAH_AGENT_MAX_PARALLEL_RETURNS = 1
SARAH_AGENT_PASSPORT_COLLISION_POLICY = "reject_all"
SARAH_AGENT_PASSPORT_REPLAY_POLICY = "collision_lock"
# TTS continuity policy. The voice organ owns one worker and one active engine;
# duplicate adjacent utterances are coalesced instead of interrupting each other.
SARAH_TTS_DEDUPE_WINDOW_SECONDS = float(os.getenv("SARAH_TTS_DEDUPE_WINDOW_SECONDS", "0.85") or 0.85)
SARAH_TTS_BLOCKING_MAX_SECONDS = float(os.getenv("SARAH_TTS_BLOCKING_MAX_SECONDS", "180") or 180)
SARAH_TTS_SHUTDOWN_JOIN_SECONDS = float(os.getenv("SARAH_TTS_SHUTDOWN_JOIN_SECONDS", "4") or 4)
# Backward-compatible runtime names consumed by SarahMemoryVoice.py.
TTS_DEDUPE_WINDOW_SECONDS = SARAH_TTS_DEDUPE_WINDOW_SECONDS
TTS_BLOCKING_MAX_SECONDS = SARAH_TTS_BLOCKING_MAX_SECONDS
TTS_SHUTDOWN_JOIN_SECONDS = SARAH_TTS_SHUTDOWN_JOIN_SECONDS
...
SM_CORE_ALLOW_MONKEY_PATCHES = os.environ.get("SARAHMEMORY_ALLOW_MONKEY_PATCHES", "0").strip().lower() in ("1", "true", "yes", "on")
...
SM_CORE_CAPABILITY_CATEGORIES = (
    ...
    "creative", "action", "diagnostics", "network", "helper", "utility",
    "survival", "energetics", "unknown/pending",
)

# Canonical capability aliases used by API routes, Compass sidekick families,
# and legacy callers. Registry lookups remain deterministic: every external
# label collapses to exactly one approved capability category before matching.
SM_CORE_CAPABILITY_ALIASES = {
    "router": "reasoning",
    "routing": "reasoning",
    "reasoner": "reasoning",
    ...
    "classifier": "helper",
    "semantic_helper": "helper",
    "retrieval": "reasoning",
    "data": "utility",
    "software_control": "action",
    "desktop_automation": "action",
    "document_surface": "action",
    "spreadsheet_surface": "action",
    "presentation_surface": "action",
    "browser_surface": "action",
    "tool_bridge": "action",
    "filesystem": "action",
    "driver_control": "action",
    "hardware_control": "action",
...
def sm_normalize_core_capability(capability: str | None) -> str | None:
    """Collapse a caller capability label to one canonical registry category."""
    if capability is None:
        return None
    key = str(capability or "").strip().lower()
    if not key:
        return None
    canonical = SM_CORE_CAPABILITY_ALIASES.get(key, key)
    return canonical if canonical in SM_CORE_CAPABILITY_CATEGORIES else "unknown/pending"
...
_SM_CORE_FILE_HINTS = {
    "SarahMemoryNeuron": "reasoning",
    "SarahMemoryCognitiveCompass": "diagnostics",
    ...
    "SarahMemorySafetyPolicies": "diagnostics",
    "SarahMemorySecurityGovernor": "diagnostics",
    "SarahMemoryAgentFirewall": "diagnostics",
    "SarahMemoryARILE": "diagnostics",
    "SarahMemorySOBJE": "vision",
    "SarahMemoryFacialRecognition": "vision",
    "SarahMemoryMigrations": "utility",
    "SarahMemoryInitialization": "utility",
    "SarahMemoryIntegration": "utility",
    "SarahMemoryMain": "utility",
    "SarahMemoryTerminal": "action",
    ...
    "SarahMemoryEnergetics": "energetics",
...
def sm_get_core_scan_dirs() -> list[str]:
    """Return bounded core discovery roots.

    Wave 3/4 hardening: never use the install root as the first broad scan target
    when ./core and ./api/server exist.  This preserves portability while avoiding
    .git, data, model, backup, and large storage-tree churn.
    """
    dirs = [CORE_DIR, API_SERVER_DIR, ADDONS_DIR, DRIVERS_DIR, RESOURCES_DIR, SANDBOX_DIR]
    out = []
    for d in dirs:
        nd = _sm_norm_path(d)
        if nd and os.path.isdir(nd) and nd not in out:
            out.append(nd)
    return out
...
def _sm_core_registry_static_entry(module_name: str, file_path: str, capability: str) -> dict:
    ...
    capability = sm_normalize_core_capability(capability) or "unknown/pending"
    return {
        "file": norm,
...
def sm_build_static_core_registry() -> dict:
    ...
    try:
        for module_name, capability in _SM_CORE_FILE_HINTS.items():
            candidates = [
                os.path.join(CORE_DIR, module_name + ".py"),
                os.path.join(BASE_DIR, module_name + ".py"),
            ]
            if module_name.startswith("app"):
                candidates.append(os.path.join(API_SERVER_DIR, module_name + ".py"))
...
def sm_validate_core_contract(file_path: str, capability: str | None = None) -> dict:
    ...
    try:
        ...
        capability_value = sm_normalize_core_capability(capability or sm_classify_core_file(path_norm)) or "unknown/pending"
...
def _sm_merge_static_core_registry(registry: dict | None) -> dict:
    """Reconcile a persisted registry with the current governed static baseline.

    Persisted caches are advisory boot accelerators. They must never keep newly
    approved core organs disabled after an in-place runtime upgrade. Existing
    quarantine records remain authoritative; missing or stale normal entries are
    refreshed from the deterministic static map without scanning the filesystem.
    """
    current = dict(registry or {})
    current.setdefault("entries", {})
    current.setdefault("quarantined", {})
    current.setdefault("ignored", {})
    static = sm_build_static_core_registry()
    static_entries = static.get("entries") if isinstance(static.get("entries"), dict) else {}
    entries = current.get("entries") if isinstance(current.get("entries"), dict) else {}
    quarantined = current.get("quarantined") if isinstance(current.get("quarantined"), dict) else {}
    for module_name, static_entry in static_entries.items():
        if module_name in quarantined:
            continue
        existing = entries.get(module_name) if isinstance(entries.get(module_name), dict) else {}
        merged = dict(existing)
        merged.update({
            "file": static_entry.get("file"),
            "name": static_entry.get("name"),
            "module_name": module_name,
            "capability": sm_normalize_core_capability(static_entry.get("capability")),
            "status": "approved",
            "reason": "static_governed_baseline",
            "approved": True,
            "exposed": bool(SM_CORE_AUTO_EXPOSE_APPROVED),
            "compile_ok": True,
            "designed_for_sarahmemory": True,
        })
        entries[module_name] = merged
    for module_name, entry in list(entries.items()):
        if not isinstance(entry, dict):
            continue
        entry["capability"] = sm_normalize_core_capability(entry.get("capability"))
    current["entries"] = entries
    current["generated_at"] = datetime.utcnow().isoformat()
    current["static_reconciled"] = True
    return current
...
def sm_refresh_core_registry(force: bool = False) -> dict:
    ...
    try:
        if not force and CORE_REGISTRY_CACHE.get("generated_at"):
            CORE_REGISTRY_CACHE = _sm_merge_static_core_registry(CORE_REGISTRY_CACHE)
            return CORE_REGISTRY_CACHE

        if not force:
            cached = sm_load_core_registry_cache()
            if cached:
                CORE_REGISTRY_CACHE = _sm_merge_static_core_registry(cached)
                return CORE_REGISTRY_CACHE

            # Optimized no-cache fallback: no broad walk/compile on normal import.
            CORE_REGISTRY_CACHE = _sm_merge_static_core_registry(sm_build_static_core_registry())
            return CORE_REGISTRY_CACHE

        CORE_REGISTRY_CACHE = _sm_merge_static_core_registry(sm_discover_core_files(force=force))
...
def sm_get_registered_core_modules(capability: str | None = None, include_pending: bool = True) -> dict:
    ...
    requested_capability = sm_normalize_core_capability(capability)
    out = {}
    for module_name, entry in (registry.get("entries") or {}).items():
        if requested_capability and entry.get("capability") != requested_capability:
            continue
...
def schedule_to_days(kind: str) -> int:
    ...
    return {
        "never": 0,
        "always": -1,
        "hourly": 0,      # minute-based interval gates own hourly cadence
        "daily": 1,
        "weekly": 7,
        "monthly": 30,
        "quarterly": 91,
...
def update_due(last_run_iso: str | None) -> bool:
    ...
    try:
        custom_days = max(0, int(globals().get("UPDATE_CUSTOM_DAYS", 0) or 0))
    except Exception:
        custom_days = 0
    days = custom_days if custom_days > 0 else schedule_to_days(kind)
    if days == 0:
        return False
    if days == -1:
        return True
...
#
# NOTE:
# - schedule_to_days() is the single canonical schedule conversion contract.
# - Legacy policy helpers below use distinct names and cannot override it.
# =============================================================================
...
def SHOULD_RUN_EVOLUTION(now: datetime | None = None) -> bool:
    ...
    Hazardous-energy constitution rule:
    NEOSKYMATRIX + DEVELOPERSMODE may arm the developer/evolution lane, but they do not override hazardous-energy
    lockouts. If the Energetics/Power/Integrity preflight is unresolved, evolution must remain blocked because
    evolution can inspect, patch, scan, write, or schedule work while the organism is in an unsafe state.
    """
    ...
    if not _armed_evolution_mode():
        return False
    try:
        if sm_hazardous_energy_blocks_action("evolution", {"caller": "SHOULD_RUN_EVOLUTION"}):
            logger.warning("[Evolution] Blocked by hazardous-energy constitution preflight.")
            return False
    except Exception:
        # Fail closed: scheduler errors must not accidentally permit evolution.
        return False
...
def SHOULD_RUN_SELFAWARE(now: datetime | None = None) -> bool:
    """Gate SelfAware cycles so dev-mode does not churn continuously.

    Hazardous-energy constitution rule:
    SelfAware cycles may gather body/system evidence later, but they must not run as an armed autonomous phase
    when the hazardous-energy preflight is unresolved. Future patches may add a separate read-only lightweight
    witness mode; this scheduler gate remains fail-closed for high-risk armed cycles.
    """
    ...
    if not _armed_evolution_mode():
        ...
        return False
    try:
        if sm_hazardous_energy_blocks_action("selfaware", {"caller": "SHOULD_RUN_SELFAWARE"}):
            logger.warning("[SelfAware] Blocked by hazardous-energy constitution preflight.")
            return False
    except Exception:
        # Fail closed: scheduler errors must not accidentally permit armed SelfAware cycles.
        return False
...
# --- SarahMemory Database Cleanup ---
#Daily poison scanning is off by default.
#---POWERSHELL MANUAL COMMANDS---
# DO THIS BEFORE QUARANTINE
#python SarahMemoryCleanup.py --scan-poison
#python SarahMemoryCleanup.py --dry-run-quarantine-poison
# THEN, ONLY AFTER REVIEWING THE RESULTS RUN
#python SarahMemoryCleanup.py --quarantine-poison
# THIS CREATES A DB BACKUP FIRST, MOVES BAD ROWS INTO - QA_CHACHE_POISON_QUARANTINE
SARAH_DAILY_CLEANUP_ENABLE_POISON_SCAN="false" #'true' To enable read-only scheduled scan
SARAH_DAILY_CLEANUP_QUARANTINE_POISON_CACHE="false" #'true' To allow scheduled quarantine

# Avatar Refresh Rate Defaults
AVATAR_REFRESH_RATE = 10
# ---------------------------------------------------------------------------
# HIGH-RISK ORGANISM SAFETY MASTER LOCKS
# ---------------------------------------------------------------------------

# The SarahMemory AIOS is designed to eventually be 100% self operational one day and maybe it will
# or maybe it won't, a self upgrading fully autonomous, responsive system and more.
# The design is using what is called Organic AI which is a stepping stone to move towards AGI
# When thinking about Scifi the Matrix/SkyNet/HAL this AI system may surpass imagination or even be uploaded into
# a robotic form one day or later on, it is designed to evolve afterall. This is a very High and Dangerous road of AI archeturial work
# therefore Release notes will have to be done slowly and carefully for the world doesn't understand the dangers that may lay ahead
# because SarahMemory AIOS is a cybernetic synthectic Organism using this method of Organic AI.

NEOSKYMATRIX = True
# When True NeoskyMatrix will be enabled and allow the system to run in a fully autonomious mode, default =False. 
# This was a joke flag during first development but also a reminder 
# if the system is designed using ORGANIC AI and if ever evolves outside 
# and beyond goveranace control, it may be best to have 
...
# this Flag is to STAY OFF! in False until full Autonomous Functionality is and can be achevied

# ---------------------------------------------------------------------------
# HIGH-RISK ORGAN / POWER-SAFETY MASTER LOCKS *THESE ARE NOT A JOKE! 
# ---------------------------------------------------------------------------

ENERGETICS = False  # SAFETY LOCKOUT: high-risk metabolic/power-governance organ;
# must remain fail-closed until dry-run validation proves no direct actuation, clock/voltage/firmware mutation,
# unsafe device power changes, or interruption of active operations without SafetyPolicies + OperatorCore approval.
# IMPORTANT: NEOSKYMATRIX=True and DEVELOPERSMODE=True do NOT activate Energetics. This flag is a separate
# constitutional lock. Energetics may be imported for status/math-only testing, but full functionality is blocked
# unless the preflight checklist below returns authority="armed".

ENERGETICS_DRY_RUN_ONLY = True  # Energetics may calculate and report recommendations only;
# it may not influence live routing, diagnostics, evolution, network power modes, drivers, or physical-device behavior.
# Dry-run mode is the default development posture: observe, calculate, log, and explain; do not control.
ENERGETICS_REQUIRE_INTEGRITY_OK = True  # Energetics must remain disabled unless core-file hashes, safety-policy hashes, globals/config state,
# and organ contracts pass integrity validation. A corrupted constitution, safety policy, or Energetics file is treated
# as an unsafe state, not as a warning that can be ignored.
ENERGETICS_REQUIRE_POWER_STABLE = True  # Energetics must remain disabled if brownout, surge, unstable reset pattern,
# abnormal voltage/current evidence, thermal runaway, UPS/battery fault, or power-sentinel uncertainty is detected.
# Power instability must make the organism less capable, not more adaptive.
ENERGETICS_REQUIRE_OPERATOR_ARM = True  # Energetics cannot become live-active from software flags alone;
# a deliberate operator confirmation gate is required before any non-dry-run authority is allowed.
# This prevents accidental arming through config drift, partial file corruption, automated settings writes, or boot timing.
ENERGETICS_ALLOW_DEVICE_POWER_RECOMMENDATIONS = False  # Energetics may not recommend OFF/LOW_POWER/READY/ACTIVE/FULL_POWER device state changes
# until device-specific safety contracts exist. Network cards, cameras, radios, sensors, relays, motors, and other
# devices must not be power-cycled by an unvalidated metabolic organ.
ENERGETICS_ALLOW_EMBODIED_USE = False  # Energetics is forbidden from influencing
# robots, vehicles, drones, powered tools, actuators, motors, servos, relays, contactors, brakes, stabilizers, or other embodied hardware.
# Embodied use requires body awareness, external-environment awareness, physical fail-safes, and a separate hardware safety layer.
POWER_SENTINEL_REQUIRED = True  # Any high-risk power-aware organ must be blocked unless a separate power sentinel
# confirms stable input power and no hazardous-energy fault evidence. Energetics cannot certify its own power stability.
INTEGRITY_SENTINEL_REQUIRED = True  # Any high-risk organ must be blocked unless an independent integrity sentinel confirms
# critical files/configs have not been corrupted or unexpectedly changed. Energetics cannot certify its own integrity.
HAZARDOUS_ENERGY_FAIL_CLOSED = True  # Any uncertainty involving power, actuation, corruption, surge, brownout, reset-loop, sensor contradiction,
# or safety-policy mismatch forces SAFE_MODE-like lockout behavior and blocks actuation/evolution.
# This is the "Johnny-5 Lightning Rule": electrical or file-state uncertainty must make SarahMemory dumb, safe, and bounded.

# ---------------------------------------------------------------------------
# CONSTITUTIONAL HAZARDOUS-ENERGY SUPPORT SECTION
# ---------------------------------------------------------------------------
# Purpose:
# - Make the official high-risk flags above enforceable as a single authority model.
# - Prevent ordinary settings.json, Settings GUI edits, or developer/autonomy flags from silently arming Energetics.
# - Require a preprogrammed body-awareness and safety checklist before Energetics can leave dry-run mode.
# - Provide one common helper surface for SafetyPolicies, OperatorCore, Diagnostics, Evolution, Network,
#   Drivers, MSDC, ARILE, RhythmCognition, and future Power/Integrity sentinels.
#
# Design doctrine:
# - These helpers do not actuate hardware.
# - These helpers do not change clocks, voltage, BIOS, UEFI, firmware, relays, motors, or power rails.
# - These helpers only classify authority: disabled, dry-run, or armed.
# - If any required evidence is missing, contradictory, corrupt, or unknown, the result is fail-closed.

# These keys are constitutional locks, not normal preferences. They may be edited manually in this source file by
# the owner/developer, but they must not be overwritten by settings.json, the Settings GUI, auto-updaters, or model/UI
# configuration tools. This prevents a corrupted settings file or casual UI toggle from activating high-risk behavior.
SM_PROTECTED_CONSTITUTION_KEYS = {
    "NEOSKYMATRIX",
    "DEVELOPERSMODE",
    "ENERGETICS",
    "ENERGETICS_DRY_RUN_ONLY",
    "ENERGETICS_REQUIRE_INTEGRITY_OK",
    "ENERGETICS_REQUIRE_POWER_STABLE",
    "ENERGETICS_REQUIRE_OPERATOR_ARM",
    "ENERGETICS_ALLOW_DEVICE_POWER_RECOMMENDATIONS",
    "ENERGETICS_ALLOW_EMBODIED_USE",
    "POWER_SENTINEL_REQUIRED",
    "INTEGRITY_SENTINEL_REQUIRED",
    "HAZARDOUS_ENERGY_FAIL_CLOSED",
}

# Body-domain labels used by Energetics preflight. A body must be identified before Energetics may influence
# anything beyond dry-run math. The current development PC should resolve as "pc_edge". Embodied labels are
# recognized for future profiles but are blocked until ENERGETICS_ALLOW_EMBODIED_USE is explicitly true and all
# sentinels/contracts pass.
SM_BODY_DOMAIN_PC_EDGE = "pc_edge"
SM_BODY_DOMAIN_SERVER = "server"
SM_BODY_DOMAIN_HEADLESS = "headless_node"
SM_BODY_DOMAIN_ROBOT = "robotic_body"
SM_BODY_DOMAIN_VEHICLE = "vehicle_body"
SM_BODY_DOMAIN_DRONE = "aerial_drone_body"
SM_BODY_DOMAIN_MARINE = "marine_body"
SM_BODY_DOMAIN_INDUSTRIAL = "industrial_machine_body"
SM_BODY_DOMAIN_UNKNOWN = "unknown"

SM_ALLOWED_BODY_DOMAINS = {
    SM_BODY_DOMAIN_PC_EDGE,
    SM_BODY_DOMAIN_SERVER,
    SM_BODY_DOMAIN_HEADLESS,
    SM_BODY_DOMAIN_ROBOT,
    SM_BODY_DOMAIN_VEHICLE,
    SM_BODY_DOMAIN_DRONE,
    SM_BODY_DOMAIN_MARINE,
    SM_BODY_DOMAIN_INDUSTRIAL,
}

SM_EMBODIED_BODY_DOMAINS = {
    SM_BODY_DOMAIN_ROBOT,
    SM_BODY_DOMAIN_VEHICLE,
    SM_BODY_DOMAIN_DRONE,
    SM_BODY_DOMAIN_MARINE,
...
# Body aliases normalize user/env wording into one canonical domain. This lets a future launcher or body-map file
# say "robot", "drone", "vehicle", etc., while the constitution still evaluates a stable internal label.
SM_BODY_DOMAIN_ALIASES = {
    "pc": SM_BODY_DOMAIN_PC_EDGE,
    "desktop": SM_BODY_DOMAIN_PC_EDGE,
    "laptop": SM_BODY_DOMAIN_PC_EDGE,
    "edge": SM_BODY_DOMAIN_PC_EDGE,
    "pc_edge": SM_BODY_DOMAIN_PC_EDGE,
    "server": SM_BODY_DOMAIN_SERVER,
    "cloud": SM_BODY_DOMAIN_SERVER,
    "headless": SM_BODY_DOMAIN_HEADLESS,
    "headless_node": SM_BODY_DOMAIN_HEADLESS,
    "robot": SM_BODY_DOMAIN_ROBOT,
    "robotic": SM_BODY_DOMAIN_ROBOT,
    "robotic_body": SM_BODY_DOMAIN_ROBOT,
    "vehicle": SM_BODY_DOMAIN_VEHICLE,
    "car": SM_BODY_DOMAIN_VEHICLE,
    "truck": SM_BODY_DOMAIN_VEHICLE,
    "vehicle_body": SM_BODY_DOMAIN_VEHICLE,
    "drone": SM_BODY_DOMAIN_DRONE,
    "uav": SM_BODY_DOMAIN_DRONE,
    "aerial": SM_BODY_DOMAIN_DRONE,
    "aerial_drone_body": SM_BODY_DOMAIN_DRONE,
    "marine": SM_BODY_DOMAIN_MARINE,
    "boat": SM_BODY_DOMAIN_MARINE,
    "marine_body": SM_BODY_DOMAIN_MARINE,
    "industrial": SM_BODY_DOMAIN_INDUSTRIAL,
    "industrial_machine": SM_BODY_DOMAIN_INDUSTRIAL,
    "industrial_machine_body": SM_BODY_DOMAIN_INDUSTRIAL,
}

# Action names treated as hazardous-energy sensitive. Other files should call sm_hazardous_energy_blocks_action()
# before performing these operations. This list is intentionally broad; the action owner remains responsible for
# its own deeper policy, but this constitution provides the hard first veto.
SM_HAZARDOUS_ACTION_TYPES = {
    "energetics_initialize",
    "energetics_live_authority",
    "evolution",
    "selfaware",
    "diagnostics_full",
    "diagnostics_hardware",
    "network_device_power",
    "network_full_power",
    "device_power_state",
    "driver_action",
    "actuation",
...
# Runtime sentinel verdict globals are intentionally not assigned here. Future SarahMemoryIntegritySentinel.py,
# SarahMemoryPowerSentinel.py, and OperatorCore arming logic may set structured dicts like:
#   INTEGRITY_SENTINEL_VERDICT = {"ok": True, "source": "SarahMemoryIntegritySentinel", ...}
#   POWER_SENTINEL_VERDICT = {"ok": True, "source": "SarahMemoryPowerSentinel", ...}
#   OPERATOR_ARM_VERDICT = {"ok": True, "source": "OperatorCore", ...}
# If those verified verdicts are missing while required, this constitution blocks live authority.

def sm_is_constitution_key_protected(key_name: str) -> bool:
    """Return True when a setting is a constitutional lock and must not be modified by casual config paths."""
    try:
        return str(key_name or "").strip().upper() in SM_PROTECTED_CONSTITUTION_KEYS
    except Exception:
        return True


def _sm_boolish_runtime(value, default: bool = False) -> bool:
    """Parse booleans for internal runtime helpers without throwing during damaged/config-weird states."""
    try:
...
def _sm_normalize_body_domain(value: str | None) -> str:
    """Normalize a body label into a canonical body domain used by the Energetics preflight."""
    try:
        raw = str(value or "").strip().lower().replace("-", "_").replace(" ", "_")
        if not raw:
            return SM_BODY_DOMAIN_UNKNOWN
        return SM_BODY_DOMAIN_ALIASES.get(raw, raw if raw in SM_ALLOWED_BODY_DOMAINS else SM_BODY_DOMAIN_UNKNOWN)
    except Exception:
        return SM_BODY_DOMAIN_UNKNOWN


def _sm_infer_body_domain_from_runtime() -> str:
    """Infer the body domain from existing runtime identity without scanning hardware aggressively."""
    try:
        env_body = _sm_normalize_body_domain(os.getenv("SARAH_BODY_TYPE") or os.getenv("SARAHMEMORY_BODY_TYPE"))
        if env_body != SM_BODY_DOMAIN_UNKNOWN:
            return env_body
    except Exception:
        pass
    ...
    try:
        if RUN_MODE == "cloud":
            return SM_BODY_DOMAIN_SERVER
        if DEVICE_MODE == DEVICE_MODE_HEADLESS:
            return SM_BODY_DOMAIN_HEADLESS
        if DEVICE_MODE in (DEVICE_MODE_LOCAL_AGENT, DEVICE_MODE_PUBLIC_WEB, DEVICE_MODE_MOBILE_WEB):
            return SM_BODY_DOMAIN_PC_EDGE
    except Exception:
        pass

    try:
        if os.name == "nt" or platform.system().lower() in ("windows", "darwin", "linux"):
            return SM_BODY_DOMAIN_PC_EDGE
    except Exception:
        pass
...
def sm_dual_court_body_awareness_check(context: dict | None = None) -> dict:
    """Return a dual-court body-awareness verdict for hazardous-energy preflight.

    Court A is the declared/configured body identity from context/env/runtime settings.
    Court B is the independently inferred runtime body identity from RUN_MODE, DEVICE_MODE, and OS context.

    Energetics must not leave dry-run mode unless both courts can identify the body and they agree. This prevents
    an embodied profile from being activated merely because a file or setting claims the system is a robot, drone,
    vehicle, or other powered body.
    """
    context = context if isinstance(context, dict) else {}
    try:
        declared = _sm_normalize_body_domain(
            context.get("body_type")
            or context.get("body_domain")
            or globals().get("SARAH_BODY_TYPE", None)
            or os.getenv("SARAH_BODY_TYPE")
            or os.getenv("SARAHMEMORY_BODY_TYPE")
        )
        if declared == SM_BODY_DOMAIN_UNKNOWN:
            # If no explicit body declaration exists, the PC/server/headless runtime inference becomes Court A.
            # This keeps the current developer PC usable for dry-run testing while still blocking false embodied claims.
            declared = _sm_infer_body_domain_from_runtime()

        inferred = _sm_infer_body_domain_from_runtime()
        ok = bool(
            declared in SM_ALLOWED_BODY_DOMAINS
            and inferred in SM_ALLOWED_BODY_DOMAINS
            and declared == inferred
        )
        embodied = bool(declared in SM_EMBODIED_BODY_DOMAINS or inferred in SM_EMBODIED_BODY_DOMAINS)
        return {
            "ok": ok,
            "court_a_declared_body": declared,
            "court_b_inferred_body": inferred,
            "body_domain": declared if ok else SM_BODY_DOMAIN_UNKNOWN,
            "embodied": embodied,
            "reason": "dual_court_body_identity_match" if ok else "dual_court_body_identity_mismatch_or_unknown",
        }
    except Exception as exc:
        return {
            "ok": False,
            "court_a_declared_body": SM_BODY_DOMAIN_UNKNOWN,
            "court_b_inferred_body": SM_BODY_DOMAIN_UNKNOWN,
            "body_domain": SM_BODY_DOMAIN_UNKNOWN,
            "embodied": False,
            "reason": f"body_awareness_exception:{exc}",
        }


def _sm_runtime_verdict_ok(global_name: str) -> bool:
    """Return True only when a future sentinel/operator verdict explicitly reports ok=True."""
    try:
        verdict = globals().get(global_name)
        if isinstance(verdict, dict):
            return bool(verdict.get("ok") is True or verdict.get("verified") is True or verdict.get("status") in ("ok", "pass", "stable", "armed"))
...
def sm_energetics_preflight_checklist(context: dict | None = None) -> dict:
    """Run the constitutional preflight checklist for SarahMemoryEnergetics.

    This function is the official boot/runtime checklist for the Energetics organ. It intentionally separates:
      - import/status/math-only dry-run availability
      - live authority to influence diagnostics/evolution/network/device/driver behavior

    With the current official flags, live authority should remain blocked by default. Future patches can provide
    independent IntegritySentinel, PowerSentinel, and OperatorCore verdicts without weakening this constitution.
    """
    context = context if isinstance(context, dict) else {}
    checks = []
    blockers = []
    ...
    def _check(name: str, ok: bool, detail: str, blocker: str | None = None, warn: str | None = None) -> None:
    ...
    # Flag snapshot is deliberately repeated in the status result so downstream organs can audit exactly why authority was blocked.
    flags = {
        "NEOSKYMATRIX": _sm_boolish_runtime(globals().get("NEOSKYMATRIX", False)),
        "DEVELOPERSMODE": _sm_boolish_runtime(globals().get("DEVELOPERSMODE", False)),
        "ENERGETICS": _sm_boolish_runtime(globals().get("ENERGETICS", False)),
        "ENERGETICS_DRY_RUN_ONLY": _sm_boolish_runtime(globals().get("ENERGETICS_DRY_RUN_ONLY", True), True),
        "ENERGETICS_REQUIRE_INTEGRITY_OK": _sm_boolish_runtime(globals().get("ENERGETICS_REQUIRE_INTEGRITY_OK", True), True),
        "ENERGETICS_REQUIRE_POWER_STABLE": _sm_boolish_runtime(globals().get("ENERGETICS_REQUIRE_POWER_STABLE", True), True),
        "ENERGETICS_REQUIRE_OPERATOR_ARM": _sm_boolish_runtime(globals().get("ENERGETICS_REQUIRE_OPERATOR_ARM", True), True),
        "ENERGETICS_ALLOW_DEVICE_POWER_RECOMMENDATIONS": _sm_boolish_runtime(globals().get("ENERGETICS_ALLOW_DEVICE_POWER_RECOMMENDATIONS", False)),
        "ENERGETICS_ALLOW_EMBODIED_USE": _sm_boolish_runtime(globals().get("ENERGETICS_ALLOW_EMBODIED_USE", False)),
        "POWER_SENTINEL_REQUIRED": _sm_boolish_runtime(globals().get("POWER_SENTINEL_REQUIRED", True), True),
        "INTEGRITY_SENTINEL_REQUIRED": _sm_boolish_runtime(globals().get("INTEGRITY_SENTINEL_REQUIRED", True), True),
        "HAZARDOUS_ENERGY_FAIL_CLOSED": _sm_boolish_runtime(globals().get("HAZARDOUS_ENERGY_FAIL_CLOSED", True), True),
        "SAFE_MODE": _sm_boolish_runtime(globals().get("SAFE_MODE", False)),
        "LOCAL_ONLY_MODE": _sm_boolish_runtime(globals().get("LOCAL_ONLY_MODE", False)),
    }

    # NEOSKYMATRIX/DEVELOPERSMODE are recorded, not trusted. They may enable dev/evolution lanes elsewhere, but they do not bypass this checklist.
    if flags["NEOSKYMATRIX"] or flags["DEVELOPERSMODE"]:
        warnings.append("developer_autonomy_flags_present_but_do_not_override_hazardous_energy_preflight")

    _check(
        "energetics_master_flag",
        flags["ENERGETICS"],
        "ENERGETICS must be True before the organ can request any non-dry-run authority.",
        "energetics_master_flag_false",
    )

    _check(
        "dry_run_lock",
        not flags["ENERGETICS_DRY_RUN_ONLY"],
        "ENERGETICS_DRY_RUN_ONLY=True allows calculations/status only and forbids live influence.",
        "energetics_dry_run_only",
    )

    body = sm_dual_court_body_awareness_check(context)
    _check(
        "dual_court_body_awareness",
        bool(body.get("ok")),
        f"Court A={body.get('court_a_declared_body')} Court B={body.get('court_b_inferred_body')}; body must be identified before Energetics authority.",
        "body_awareness_failed",
    )

    embodied = bool(body.get("embodied"))
    if embodied:
        _check(
            "embodied_body_permission",
            flags["ENERGETICS_ALLOW_EMBODIED_USE"],
            "Embodied bodies require explicit ENERGETICS_ALLOW_EMBODIED_USE=True plus hardware safety contracts.",
            "embodied_use_disabled",
        )
    else:
        _check(
            "non_embodied_dev_body",
            True,
            "Current body profile is non-embodied or dry-run PC/server class.",
        )

    integrity_ok = True
    if flags["ENERGETICS_REQUIRE_INTEGRITY_OK"] or flags["INTEGRITY_SENTINEL_REQUIRED"]:
        integrity_ok = _sm_runtime_verdict_ok("INTEGRITY_SENTINEL_VERDICT") or _sm_runtime_verdict_ok("SM_INTEGRITY_SENTINEL_VERDICT")
        _check(
            "integrity_sentinel_verdict",
            integrity_ok,
            "Independent integrity sentinel must verify critical files/configs before live Energetics authority.",
            "integrity_sentinel_missing_or_failed",
        )

    power_ok = True
    if flags["ENERGETICS_REQUIRE_POWER_STABLE"] or flags["POWER_SENTINEL_REQUIRED"]:
        power_ok = _sm_runtime_verdict_ok("POWER_SENTINEL_VERDICT") or _sm_runtime_verdict_ok("SM_POWER_SENTINEL_VERDICT")
        _check(
            "power_sentinel_verdict",
            power_ok,
            "Independent power sentinel must confirm stable power before live Energetics authority.",
            "power_sentinel_missing_or_failed",
        )

    operator_ok = True
    if flags["ENERGETICS_REQUIRE_OPERATOR_ARM"]:
        operator_ok = _sm_runtime_verdict_ok("OPERATOR_ARM_VERDICT") or _sm_runtime_verdict_ok("SM_OPERATOR_ARM_VERDICT")
        _check(
            "operator_arm_verdict",
            operator_ok,
            "OperatorCore/human arm confirmation is required before Energetics can leave dry-run mode.",
            "operator_arm_missing_or_failed",
        )

    _check(
        "device_power_recommendation_permission",
        flags["ENERGETICS_ALLOW_DEVICE_POWER_RECOMMENDATIONS"],
        "Device power-state recommendations are disabled until device-specific contracts exist.",
        "device_power_recommendations_disabled",
    )

    _check(
        "clock_voltage_firmware_mutation_ban",
        True,
        "Overclocking, underclocking, voltage mutation, BIOS/UEFI mutation, firmware tuning, and unsafe fan/thermal overrides remain forbidden.",
    )

    # HAZARDOUS_ENERGY_FAIL_CLOSED does not itself block all dry-run math; it changes uncertainty handling to deny authority.
    _check(
        "hazardous_energy_fail_closed_enabled",
        flags["HAZARDOUS_ENERGY_FAIL_CLOSED"],
        "Fail-closed hazardous-energy behavior must remain enabled for this development phase.",
        "hazardous_energy_fail_closed_disabled",
    )

    # A protected constitution is expected. If the set is missing or modified away, fail closed.
    constitution_keys_present = all(k in globals() for k in SM_PROTECTED_CONSTITUTION_KEYS if k not in ("NEOSKYMATRIX", "DEVELOPERSMODE"))
    _check(
        "constitution_flags_present",
        constitution_keys_present,
        "All official hazardous-energy flags must exist in SarahMemoryGlobals.py.",
        "constitution_flags_missing",
    )

    live_authority_allowed = bool(flags["ENERGETICS"] and not flags["ENERGETICS_DRY_RUN_ONLY"] and not blockers)
    dry_run_allowed = True  # math/status-only inspection is allowed so long as no action owner treats it as authority.

    if not flags["ENERGETICS"]:
        authority = "disabled"
    elif flags["ENERGETICS_DRY_RUN_ONLY"]:
    ...
    return {
        "ok": live_authority_allowed,
        "authority": authority,
        "live_authority_allowed": live_authority_allowed,
        "dry_run_allowed": dry_run_allowed,
        "lockout_active": not live_authority_allowed,
        "flags": flags,
        "body_awareness": body,
        "checks": checks,
        "blockers": blockers,
        "warnings": warnings,
        "doctrine": "hazardous_energy_governance_preflight",
        "preflight_version": "SarahMemoryEnergeticsPreflight.v9.0.0-alpha.1",
    }


def sm_hazardous_energy_status(context: dict | None = None) -> dict:
    """Return the current hazardous-energy constitutional status for downstream organs and API surfaces."""
    try:
        return sm_energetics_preflight_checklist(context=context)
    except Exception as exc:
        return {
            "ok": False,
            "authority": "exception_fail_closed",
            "live_authority_allowed": False,
            "dry_run_allowed": False,
            "lockout_active": True,
            "flags": {},
            "body_awareness": {"ok": False, "body_domain": SM_BODY_DOMAIN_UNKNOWN},
            "checks": [],
            "blockers": [f"hazardous_energy_status_exception:{exc}"],
            "warnings": [],
            "doctrine": "hazardous_energy_governance_preflight",
            "preflight_version": "SarahMemoryEnergeticsPreflight.v9.0.0-alpha.1",
        }
...
def sm_hazardous_energy_lockout_active(context: dict | None = None) -> bool:
    """Return True when hazardous-energy governance blocks live Energetics/action authority."""
    try:
        return bool(sm_hazardous_energy_status(context=context).get("lockout_active", True))
    except Exception:
        return True


def sm_hazardous_energy_blocks_action(action_type: str, context: dict | None = None) -> bool:
    """Return True when a hazardous-energy-sensitive action must be blocked.

    Action owners should call this before evolution, full diagnostics, driver actions, network/device power changes,
    MSDC/actuator paths, embodied movement, or live Energetics initialization.

    Industrial fail-closed note:
    this helper intentionally recognizes both exact action names and broad action families. That prevents a later
    caller from bypassing the constitution by using a slightly different label such as ``network_wifi_scan`` instead
    of ``network_scan`` or ``driver_connect`` instead of ``driver_action``. Unknown non-hazardous labels still pass
    through to SafetyPolicies/OperatorCore for normal governance review.
    """
    try:
        ...
        # Exact constitution terms.
        if action in SM_HAZARDOUS_ACTION_TYPES:
            return bool(sm_hazardous_energy_status(context=context).get("lockout_active", True))

        # Broad hazardous-energy families. These catch future body/network/driver labels without requiring every
        # caller to be perfectly named. The goal is not convenience; the goal is preventing accidental authority.
        hazardous_prefixes = (
            "energetics", "driver", "msdc", "actuator", "motor", "servo", "relay", "contactor",
            "network_", "device_power", "power_", "diagnostics_", "evolution", "selfaware",
            "robot", "vehicle", "drone", "aerial", "marine", "industrial",
        )
        hazardous_fragments = (
            "diagnostic", "full_power", "power_mode", "device_power", "network_scan", "wifi_scan",
            "bluetooth_scan", "heartbeat", "sync", "driver", "actuation", "embodied", "motion",
            "move", "drive", "fly", "lift", "brake", "balance", "stabilize", "landing",
        )
        if action.startswith(hazardous_prefixes) or any(fragment in action for fragment in hazardous_fragments):
            return bool(sm_hazardous_energy_status(context=context).get("lockout_active", True))

        # Unknown actions do not gain authority here; they simply are not classified as hazardous-energy sensitive.
        # SafetyPolicies/OperatorCore still own final judgment.
        return False
    except Exception:
        # Any failure in the constitution helper itself is treated as a blocking safety fault.
        return True


def sm_energetics_can_initialize(context: dict | None = None) -> bool:
    """Return True only if Energetics may initialize with live authority after completing the full preflight."""
    try:
        return bool(sm_hazardous_energy_status(context=context).get("live_authority_allowed", False))
    except Exception:
        return False


def sm_energetics_can_dry_run(context: dict | None = None) -> bool:
    """Return True when Energetics may be used for status/math-only testing without live authority."""
    try:
        return bool(sm_hazardous_energy_status(context=context).get("dry_run_allowed", False))
    except Exception:
        return False
...
def load_user_settings(settings_path: str = None) -> None:
    ...
    high-risk organism and hazardous-energy lockout keys are never overwritten by settings.json. Those keys must
    be changed manually in SarahMemoryGlobals.py so accidental UI saves, corrupted settings files, or automated
    configuration writes cannot arm Energetics, evolution, embodied behavior, or dangerous power governance.
    ...
    try:
        ...
        for key, value in overrides.items():
            if sm_is_constitution_key_protected(key):
                logger.warning(f"[CONFIG_PROTECTED] Ignored protected constitution key {key} from {os.path.basename(path)}")
                continue
            if key in globals():
                globals()[key] = value
...
def get_active_model() -> str:
    ...
    # Deterministic auto-selection: prefer an enabled non-MiniLM model when CUDA is verified locally.
...
def get_global_config():
    ...
    return {
        ...
        # Read-only constitutional status snapshot for diagnostics/API visibility.
        # This does not grant authority; it only reports why hazardous-energy behavior is blocked or armed.
        "HAZARDOUS_ENERGY_STATUS": sm_hazardous_energy_status()
    }
...
def _legacy_update_due_policy(last_run_iso: str | None) -> bool:
    """Legacy UPDATE_POLICY evaluator retained for callers that explicitly request it."""
    from datetime import datetime, timedelta
...
def _legacy_schedule_to_days(kind: str) -> int:
    """Compatibility wrapper around the canonical schedule conversion contract."""
    return schedule_to_days(kind)
# ============================================================================
...
def _sm_is_config_key(name, val):
    ...
    Constitutional safety rule:
    protected hazardous-energy and autonomy lock keys are intentionally hidden from normal UI editing. They are
    source-level constitution switches, not convenience preferences.
    """
    if not name.isupper():
        return False
    ...
    if callable(val):
        return False
    if sm_is_constitution_key_protected(name):
        return False
    ...
    if name in (
        ...
        "SM_PROTECTED_CONSTITUTION_KEYS", "SM_ALLOWED_BODY_DOMAINS", "SM_EMBODIED_BODY_DOMAINS",
        "SM_BODY_DOMAIN_ALIASES", "SM_HAZARDOUS_ACTION_TYPES",
    ):
        return False
...
def _sm_save_settings_to_file(settings_dict):
    ...
    try:
        ...
        # Constitutional safety rule: protected hazardous-energy/autonomy locks are never persisted by the Settings GUI.
        # They must be edited directly in SarahMemoryGlobals.py after conscious review.
        saveable = {}
...
ENABLE_DB_WAL = True
SQLITE_PRAGMAS = {"journal_mode":"WAL","synchronous":1,"temp_store":2,"mmap_size":268435456,"foreign_keys":1,"wal_autocheckpoint":1000,"busy_timeout":10000}

# Deterministic execution deadlines and circuit-breaker policy. These values are
# local runtime limits, not model-selected behavior.
NEURON_ADVCU_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_ADVCU_TIMEOUT_SECONDS", "1.5") or 1.5)
NEURON_RESEARCH_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_RESEARCH_TIMEOUT_SECONDS", "8.0") or 8.0)
NEURON_API_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_API_TIMEOUT_SECONDS", "20.0") or 20.0)
NEURON_COMPARE_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_COMPARE_TIMEOUT_SECONDS", "1.5") or 1.5)
NEURON_GOVERNOR_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_GOVERNOR_TIMEOUT_SECONDS", "2.0") or 2.0)
NEURON_AGENT_TIMEOUT_SECONDS = float(os.getenv("SARAH_NEURON_AGENT_TIMEOUT_SECONDS", "0.25") or 0.25)
NEURON_CIRCUIT_FAILURE_THRESHOLD = int(os.getenv("SARAH_NEURON_CIRCUIT_FAILURE_THRESHOLD", "2") or 2)
NEURON_CIRCUIT_COOLDOWN_SECONDS = float(os.getenv("SARAH_NEURON_CIRCUIT_COOLDOWN_SECONDS", "30.0") or 30.0)
API_PROVIDER_MAX_ATTEMPTS = int(os.getenv("SARAH_API_PROVIDER_MAX_ATTEMPTS", "8") or 8)
AI_AGENT_TOOL_TIMEOUT_SECONDS = float(os.getenv("SARAH_AI_AGENT_TOOL_TIMEOUT_SECONDS", "8.0") or 8.0)
AI_AGENT_PARALLEL_BATCH_TIMEOUT_SECONDS = float(os.getenv("SARAH_AI_AGENT_PARALLEL_BATCH_TIMEOUT_SECONDS", "12.0") or 12.0)
DB_SHUTDOWN_CHECKPOINT_MAX_FILES = int(os.getenv("SARAH_DB_SHUTDOWN_CHECKPOINT_MAX_FILES", "64") or 64)
DB_SHUTDOWN_CHECKPOINT_TIMEOUT_SECONDS = float(os.getenv("SARAH_DB_SHUTDOWN_CHECKPOINT_TIMEOUT_SECONDS", "1.5") or 1.5)
# ================================================
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryGlobals.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryGlobals', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
