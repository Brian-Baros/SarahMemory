...
def _energetics_basis(context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Read Energetics reserve/cadence guidance without granting action authority.

    RhythmCognition may reduce cadence during hazardous-energy lockout, but it may not
    interpret an Energetics import, status response, or missing bridge as permission to
    increase pulse rate or enable full-power background activity.
    """
    ctx = dict(context or {}) if isinstance(context, dict) else {}
    ctx.setdefault("source", "SarahMemoryRhythmCognition")
    try:
        blocker_fn = getattr(config, "sm_hazardous_energy_blocks_action", None)
        status_fn = getattr(config, "sm_hazardous_energy_status", None)
        if callable(blocker_fn) and blocker_fn("energetics_live_authority", {**ctx, "action_type": "rhythm_cadence"}):
            status = status_fn(ctx) if callable(status_fn) else {}
            return {
                "ok": False,
                "decision": "REDUCE_MODE",
                "allowed_power_mode": "LOW_POWER",
                "reason": "Hazardous-energy constitution blocks Energetics live cadence authority; Rhythm must stay conservative.",
                "constitution": status,
                "source": "rhythm_constitution_lockout",
            }
    except Exception as exc:
        return {"ok": False, "decision": "REDUCE_MODE", "allowed_power_mode": "LOW_POWER", "reason": f"Rhythm hazardous-energy check failed closed: {exc}", "source": "rhythm_fallback"}
    try:
        ...
        if callable(fn):
            return fn(
                "rhythm_cadence",
                phase=str(ctx.get("phase") or ctx.get("task_type") or "rhythm"),
                requested_power_mode=str(ctx.get("requested_power_mode") or "ACTIVE"),
                context=ctx,
            )
        status_fn = getattr(_Energetics, "get_energetics_status", None)
        if callable(status_fn):
            status = status_fn(ctx)
            return {"ok": False, "decision": "REDUCE_MODE", "allowed_power_mode": "LOW_POWER", "status": status}
    except Exception as exc:
        return {"ok": False, "decision": "REDUCE_MODE", "allowed_power_mode": "LOW_POWER", "reason": str(exc), "source": "rhythm_fallback"}
    return {"ok": False, "decision": "REDUCE_MODE", "allowed_power_mode": "LOW_POWER", "reason": "Energetics bridge unavailable"}
...
def build_rhythm_cadence_packet(context: Optional[Dict[str, Any]] = None, *, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    energetics = _energetics_basis(ctx)
    energy_mode = str(energetics.get("allowed_power_mode") or "ACTIVE").upper()
    ...
    if energy_decision in {"DEFER", "DENY", "REDUCE_MODE"} or energy_mode in {"LOW_POWER", "RECOVERY"}:
        params["heartbeat"] = max(float(params.get("heartbeat", 1.0)), 3.0)
        params["thinker"] = max(float(params.get("thinker", 2.0)), 6.0)
        params["agent"] = max(float(params.get("agent", 1.0)), 4.0)
        params["living"] = max(float(params.get("living", 2.5)), 8.0)
        params["writes"] = min(int(params.get("writes", 4)), 2)
        params["cycles"] = min(int(params.get("cycles", 3)), 2)
        params["backoff"] = max(float(params.get("backoff", 1.0)), 1.8)
        energy_notes.append("Energetics requested reduced cadence/reserve-preserving rhythm.")
    elif energy_mode == "READY":
        params["heartbeat"] = max(float(params.get("heartbeat", 1.0)), 2.0)
        params["writes"] = min(int(params.get("writes", 4)), 4)
        energy_notes.append("Energetics selected READY mode; cadence stays warm but bounded.")
    elif energy_mode == "FULL_POWER":
        energy_notes.append("Energetics allowed FULL_POWER, but Rhythm still has no execution authority.")
    ...
    packet = RhythmCadencePacket(
        ...
        system_pressure={**system, "energetics": energetics},
        notes=urgency_notes + mode_notes + energy_notes,
        constraints={
            ...
            "energetics_mode": energy_mode,
            "energetics_decision": energy_decision,
            "energy_is_not_execution_authority": True,
        },
    ).to_dict()
...
SML_ORGAN_METADATA = {
    "name": 'SarahMemoryRhythmCognition',
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryRhythmCognition.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryRhythmCognition',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryRhythmCognition', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
