...
try:
    from SarahMemoryCanvasStudio import CanvasStudio, Canvas, CanvasLayer as Layer
...
def ensure_video_directories() -> None:
    """Create video workspace directories only when the editor is explicitly initialized."""
    for directory in [
        CANVAS_DIR, EXPORTS_DIR, VIDEO_PROJECTS_DIR, VIDEO_TEMPLATES_DIR,
        VIDEO_CACHE_DIR, VIDEO_DIR, VIDEO_INPUTS_DIR, VIDEO_OUTPUTS_DIR,
        VIDEO_THUMBNAILS_DIR, VIDEO_AUDIO_DIR, VIDEO_EFFECTS_DIR,
    ]:
        try:
            os.makedirs(directory, exist_ok=True)
        except Exception as exc:
            logging.warning(f"[VideoEditor] Could not create directory {directory}: {exc}")
...
class VideoEditorCore:
    ...
    def __init__(self):
        """Initialize the Video Editor Core with bounded worker resources."""
        ...
        try:
            configured_threads = int(getattr(SMG, "CREATIVE_MAX_THREADS", configured_threads)) if SMG is not None else configured_threads
        except Exception:
            configured_threads = 4
        try:
            configured_threads = int(os.getenv("SARAH_CREATIVE_MAX_THREADS", str(configured_threads)) or configured_threads)
        except Exception:
            pass
        self.max_threads = max(1, min(detected_threads, configured_threads, 8))
    ...
    def build_output_manifest(self, project: VideoProject, output_path: str = "") -> Dict[str, Any]:
        """Build a bounded video-output manifest without rendering media."""
        return {
            "schema": "SARAHMEMORY_VIDEO_OUTPUT_V1",
            "project_id": getattr(project, "project_id", ""),
            "name": getattr(project, "name", ""),
            "resolution": list(getattr(project, "resolution", (0, 0)) or (0, 0)),
            "fps": int(getattr(project, "fps", 0) or 0),
            "duration": float(getattr(project, "duration", 0.0) or 0.0),
            "video_clip_count": len(getattr(project, "video_clips", []) or []),
            "audio_track_count": len(getattr(project, "audio_tracks", []) or []),
    ...
    def enterprise_self_test(self) -> Dict[str, Any]:
        """Run bounded in-memory editor invariants; no decode, render, or file write."""
        checks: List[Dict[str, Any]] = []
        project = VideoProject("self_test", "EnterpriseSelfTest", (320, 180), 30, 2.0)
        checks.append({"name": "project_dimensions", "passed": project.resolution == (320, 180), "observed": project.resolution})
        checks.append({"name": "project_fps", "passed": project.fps == 30, "observed": project.fps})
        manifest = self.build_output_manifest(project)
        checks.append({"name": "manifest_contract", "passed": manifest.get("schema") == "SARAHMEMORY_VIDEO_OUTPUT_V1", "observed": manifest})
        checks.append({"name": "bounded_threads", "passed": 1 <= self.max_threads <= 8, "observed": self.max_threads})
...
def get_video_editor_capabilities() -> Dict[str, Any]:
    """Read-only module capability report without initializing media engines."""
    return {
        "ok": True,
        "module": "SarahMemoryVideoEditorCore",
        "version": VIDEO_EDITOR_VERSION,
        "build": VIDEO_EDITOR_BUILD,
        "opencv_available": cv2 is not None,
        "wave_available": bool(WAVE_AVAILABLE),
        "tts_available": bool(TTS_AVAILABLE),
        "pydub_available": bool(PYDUB_AVAILABLE),
        "ffmpeg_available": bool(shutil.which("ffmpeg")),
        "ffprobe_available": bool(shutil.which("ffprobe")),
        "canvas_studio_available": bool(CANVAS_STUDIO_AVAILABLE),
        "music_generator_available": bool(MUSIC_GENERATOR_AVAILABLE),
        "lyrics_to_song_available": bool(LYRICS_TO_SONG_AVAILABLE),
...
SML_ORGAN_METADATA = {
    "name": 'SarahMemoryVideoEditorCore',
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryVideoEditorCore.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryVideoEditorCore',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryVideoEditorCore', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
