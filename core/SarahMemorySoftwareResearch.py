...
SML_ORGAN_METADATA = {
    "name": 'SarahMemorySoftwareResearch',
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemorySoftwareResearch.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemorySoftwareResearch',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemorySoftwareResearch', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
