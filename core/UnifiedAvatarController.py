...
def _sm_avatar_base_dir() -> str:
    """Resolve SarahMemory install root without trusting process cwd."""
    try:
        base = getattr(globals_module, 'BASE_DIR', None)
        if base:
...
def _sm_avatar_data_dir() -> str:
    try:
        data = getattr(globals_module, 'DATA_DIR', None)
        if data:
            return os.path.abspath(os.path.expanduser(str(data)))
    except Exception:
        pass
    return os.path.join(_sm_avatar_base_dir(), 'data')

def _sm_avatar_resources_dir() -> str:
    try:
        resources = getattr(globals_module, 'RESOURCES_DIR', None)
        if resources:
            return os.path.abspath(os.path.expanduser(str(resources)))
    except Exception:
        pass
    return os.path.join(_sm_avatar_base_dir(), 'resources')

def _sm_avatar_datasets_dir() -> str:
    try:
        datasets = getattr(globals_module, 'DATASETS_DIR', None)
        if datasets:
            return os.path.abspath(os.path.expanduser(str(datasets)))
    except Exception:
        pass
    return os.path.join(_sm_avatar_data_dir(), 'memory', 'datasets')
...
AVATAR_MODELS_DIR = getattr(
    globals_module,
    'AVATAR_MODELS_DIR',
    os.path.join(_sm_avatar_data_dir(), 'avatar', 'models')
)

AVATAR_DIR = getattr(
    globals_module,
    'AVATAR_DIR',
    os.path.join(_sm_avatar_resources_dir(), 'avatars')
)

AVATAR_3D_DIR = getattr(
    globals_module,
    'AVATAR_3D_DIR',
    os.path.join(_sm_avatar_resources_dir(), 'avatars', '3D')
)

SANDBOX_DIR = getattr(
    globals_module,
    'SANDBOX_DIR',
    os.path.join(_sm_avatar_base_dir(), 'sandbox')
)

# Create directories
for directory in [AVATAR_MODELS_DIR, AVATAR_DIR, AVATAR_3D_DIR, SANDBOX_DIR]:
    os.makedirs(directory, exist_ok=True)

# SARAHMEMORY_PATCH_NOTE: avatar paths use governed BASE_DIR/DATA_DIR for portability.
# =============================================================================
...
class UnifiedAvatarController:
    def _ensure_avatar_state_db(self):
        ...
        try:
            import sqlite3
            base = getattr(globals_module, "DATASETS_DIR", _sm_avatar_datasets_dir())
    ...
    def _build_rem_snapshot(self) -> Dict[str, Any]:
        ...
        base_dir = Path(_sm_avatar_base_dir())
    ...
    def get_avatar_3d_runtime_spec(self) -> Dict[str, Any]:
        """Return the governed 3D Avatar Organ runtime spec.

        This is a software visual-interface packet only.  Future physical
        humanoid robot actuation must be mediated by a separate Avatar-to-MSDC
        bridge and the normal SarahMemory governance gates.
        """
        try:
            base = Path(AVATAR_3D_DIR).expanduser().resolve()
            candidates = [
                base / "SarahMemoryAvatar_RigBootstrap.glb",
                base / "default" / "SarahMemoryAvatar_RigBootstrap.glb",
                base / "sarahmemory_3d_avatar.glb",
                base / "sarahmemory_happy_face_ball.glb",
            ]
            model_path = next((p for p in candidates if p.is_file()), candidates[0])
            exists = model_path.is_file()
            return {
                "renderMode": "gltf_model" if exists else "gold_standard_avatar",
                "modelUrl": f"/api/avatar/3d/{model_path.name}" if exists else "",
                "modelFile": model_path.name if exists else "",
                "assetContract": "BASE_DIR/resources/avatars/3D",
                "loaderState": "3D_READY_GOLDSTANDARD_EMBODIED_ENTITY" if exists else "3D_FAILED_FALLBACK_GOLD_REFERENCE",
                "pose": "stand",
                "gesture": "none",
                "lookAt": {"x": 0, "y": 1.45, "z": 0},
                "expression": self.last_emotion,
                "speaking": False,
                "listening": False,
                ...
                "meshFallbackUrl": f"/api/avatar/3d/{model_path.name}" if exists else "",
    ...
    def get_avatar_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "version": "9.0.0",
                "avatar_3d_dir": str(Path(AVATAR_3D_DIR).expanduser().resolve()),
                "avatar_3d_spec": self.get_avatar_3d_runtime_spec(),
                "timestamp": datetime.now().isoformat(),
    ...
    def _load_cascade():
        ...
        try:
            ...
            if not os.path.exists(cascade_path):
                # SARAHMEMORY_PATCH_NOTE: portable cascade fallback.
                # Do not hardcode <PROJECT_ROOT>; this system may run from any drive letter/root.
                cascade_path = os.path.join(_sm_avatar_resources_dir(), "cascades", "haarcascade_frontalface_default.xml")
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'UnifiedAvatarController.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'UnifiedAvatarController',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='UnifiedAvatarController', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
