...
try:
    import SarahMemoryEnergetics as SMENERGETICS  # type: ignore
except Exception:
    SMENERGETICS = None
...
# =============================================================================
# ENERGETICS PREFLIGHT — SCHEDULES ARE PROPOSALS, NOT COMMANDS
# =============================================================================
def _evolution_energetics_preflight(autonomous: bool = False, weekly_gate: bool = False) -> Dict[str, Any]:
    """Ask the survival/metabolic organ whether an evolution cycle may run.

    Evolution is potentially write-heavy and can trigger diagnostics, backups, DB
    scans, log scans, network snapshots, sandbox work, and repair outbox writes.
    It must not interrupt active embodied operations or run without reserve surplus.

    Industrial safety rule: evolution is never permitted by Energetics availability alone.
    SarahMemoryGlobals hazardous-energy constitution must clear first; otherwise this
    function fails closed even when NEOSKYMATRIX and DEVELOPERSMODE are True.
    """
    ctx = {"action_type": "evolution", "phase": "self_evolution", "autonomous": autonomous, "weekly_gate": weekly_gate, "source": "SarahMemoryEvolution"}
    try:
        blocker_fn = getattr(config, "sm_hazardous_energy_blocks_action", None)
        status_fn = getattr(config, "sm_hazardous_energy_status", None)
        if callable(blocker_fn) and blocker_fn("evolution", ctx):
            status = status_fn(ctx) if callable(status_fn) else {}
            return {
                "ok": False,
                "decision": "DENY",
                "reason": "Hazardous-energy constitution blocks evolution until body, integrity, power, and operator preflight clear.",
                "allowed_power_mode": "LOW_POWER",
                "constitution": status,
                "source": "SarahMemoryEvolution._evolution_energetics_preflight",
            }
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DENY",
            "reason": f"Evolution hazardous-energy constitution check failed closed: {exc}",
            "allowed_power_mode": "LOW_POWER",
            "source": "SarahMemoryEvolution._evolution_energetics_preflight",
        }
    if SMENERGETICS is None:
        return {
            "ok": False,
            "decision": "DENY",
            "reason": "Energetics unavailable; evolution cannot fail open under hazardous-energy governance.",
            "allowed_power_mode": "LOW_POWER",
            "source": "SarahMemoryEvolution._evolution_energetics_preflight",
        }
    try:
        fn = getattr(SMENERGETICS, "preflight_evolution_cycle", None)
        if callable(fn):
            return fn(autonomous=autonomous, weekly_gate=weekly_gate)
    except Exception as exc:
        return {
            "ok": False,
            "decision": "DENY",
            "reason": f"Energetics preflight failed closed for evolution: {exc}",
            "allowed_power_mode": "LOW_POWER",
        }
    return {"ok": False, "decision": "DENY", "reason": "Energetics bridge missing; evolution fails closed.", "allowed_power_mode": "LOW_POWER"}


def _print_evolution_deferred_by_energetics(verdict: Dict[str, Any]) -> None:
    try:
        print("[ENERGETICS] Evolution deferred/denied before diagnostics or patch work.")
        print(json.dumps(verdict, indent=2, ensure_ascii=False, default=str))
    except Exception:
        print(f"[ENERGETICS] Evolution deferred/denied: {verdict}")
...
def evolve_once(autonomous: bool = False, weekly_gate: bool = False) -> None:
    ...
    _energy_verdict = _evolution_energetics_preflight(autonomous=autonomous, weekly_gate=weekly_gate)
    if str(_energy_verdict.get("decision") or "ALLOW").upper() in {"DENY", "DEFER", "REDUCE_MODE"} or not bool(_energy_verdict.get("ok", True)):
        _print_evolution_deferred_by_energetics(_energy_verdict)
        return
    ...
    report = {
        ...
        "energetics_preflight": _energy_verdict,
    }
...
# -----------------------------------------------------------------------------
# SARAH_REM_PROMOTION_V1
# REM promotion/staging policy. Never patches SarahMemoryGlobals.py. In this
# initial implementation, auto-apply is restricted to metadata-only learning.
# -----------------------------------------------------------------------------
def rem_promote_or_stage(candidate: Dict[str, Any], *, snapshot: Optional[Dict[str, Any]] = None, sandbox: Optional[Dict[str, Any]] = None, governance: Optional[Dict[str, Any]] = None, assurance: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryEvolution.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryEvolution',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryEvolution', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
