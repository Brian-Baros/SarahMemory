...
# -----------------------------------------------------------------------------
# Neuron Axis / LogicCalc lane governance bridge
# -----------------------------------------------------------------------------
def evaluate_neuron_axis_lane_change(
    current_lane: str,
    requested_lane: str,
    *,
    ...
    """Evaluate a requested lane transition using LogicCalc advisory math.

    CognitiveCompass owns orientation and anti-drift. LogicCalc supplies the
    deterministic scoring math only. The returned packet is audit evidence for
    Neuron/Compass/Compare; it is not an autonomous execution authority.
    """
    cur = str(current_lane or "answer").strip().lower()
    req = str(requested_lane or "answer").strip().lower()
    same_lane = cur == req
    try:
        from SarahMemoryLogicCalc import LogicCalc as _LC  # type: ignore
        gate = getattr(_LC, "neuron_axis_gate", None)
        if callable(gate):
            requested_validity = 0.92 if same_lane else (0.72 if req in {"answer", "creative"} else 0.56)
            current_confidence = 0.94 if cur else 0.72
            try:
                out = gate(
                    current_lane_confidence=current_confidence,
                    requested_lane_validity=requested_validity,
                    governance_score=max(0.0, min(1.0, float(governance_score))),
                ...
                out = gate(
                    current_lane_confidence=current_confidence,
                    requested_lane_validity=requested_validity,
                    governance_modifier=max(0.0, min(1.0, float(governance_score))),
                    risk_penalty=max(0.0, min(1.0, float(risk_score))),
                    threshold=threshold,
                )
            if isinstance(out, dict):
                out.setdefault("current_lane", cur)
                out.setdefault("requested_lane", req)
                out.setdefault("same_lane", same_lane)
                out.setdefault("compass_role", "orientation_and_anti_drift")
                out.setdefault("logiccalc_role", "advisory_math_only")
                out.setdefault("context", dict(context or {}))
                return out
    except Exception as exc:
        return {
            "ok": False,
            "decision": 0 if req in {"action", "network", "system"} else 1,
            "verdict": "DENY_UNVERIFIED_LANE" if req in {"action", "network", "system"} else "ALLOW_ANSWER_LANE",
            "error": str(exc),
            "current_lane": cur,
            "requested_lane": req,
            "logiccalc_role": "unavailable",
        }
    return {
        "ok": False,
        "decision": 0 if req in {"action", "network", "system"} else 1,
...
def validate_agent_passport_lane(packet: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Fail-closed lane gate for returned external-agent evidence.

    A passport proves identity/scope only. It never makes the external agent an
    actor inside Neuron and never grants action, shell, network, filesystem, or
    device authority. Only a sanitized, user-reviewed evidence packet may enter
    answer/research cognition.
    """
    pkt = packet if isinstance(packet, dict) else {}
    passport = pkt.get("passport") if isinstance(pkt.get("passport"), dict) else {}
    passport_id = str(passport.get("passport_id") or pkt.get("passport_id") or "").strip()
    if not passport_id:
        return {
            "ok": False,
            "applicable": False,
            "decision": "DENY_NO_PASSPORT",
            "reply_allowed": False,
            "execution_authority": False,
            "reason": "agent_passport_missing",
        }
    requested_lane = str(pkt.get("requested_lane") or pkt.get("lane") or passport.get("origin_lane") or "answer").strip().lower()
    allowed_lanes = {str(x).strip().lower() for x in (passport.get("allowed_lanes") or []) if str(x).strip()}
    review_status = str(pkt.get("review_status") or pkt.get("user_review") or "").strip().lower()
    passport_status = str(passport.get("status") or pkt.get("passport_status") or "").strip().lower()
    sanitized = bool(pkt.get("sanitized", False))
    failures: List[str] = []
    if review_status not in {"approved", "accepted", "user_approved", "reviewed_accepted"}:
        failures.append("user_review_not_approved")
    if passport_status not in {"return_captured", "consumed"}:
        failures.append("passport_not_in_reviewed_return_state")
    if requested_lane not in allowed_lanes:
        failures.append("passport_lane_scope_mismatch")
    if requested_lane in {"action", "system", "network", "device", "shell", "filesystem", "operator"}:
        failures.append("returned_agent_evidence_cannot_enter_execution_lane")
    if not sanitized:
        failures.append("agent_payload_not_sanitized")
    if bool(pkt.get("execution_authority", False)):
        failures.append("agent_claimed_execution_authority")

    if failures:
        return {
            "ok": False,
            "applicable": True,
            "decision": "DENY_AGENT_LANE",
    ...
    return {
        "ok": True,
        "applicable": True,
        "decision": "ALLOW_EVIDENCE_ONLY",
        "verdict": "ALLOW_SANITIZED_AGENT_EVIDENCE",
        "reply_allowed": True,
        "execution_authority": False,
        "passport_id": passport_id,
        "requested_lane": requested_lane,
        "allowed_lanes": sorted(allowed_lanes),
        "requires_compare": bool(passport.get("requires_compare", True)),
        "requires_assurance": bool(passport.get("requires_assurance", True)),
        "requires_compass": True,
        "reason": "passport_scoped_sanitized_user_reviewed_evidence",
    }
...
SML_ORGAN_METADATA = {
    "name": 'SarahMemoryCognitiveCompass',
    "version": "v9.0.0-alpha-sml-0.2",
    "category": 'Governance',
    "protocol_version": "SML/1.0",
    "packet_version": 1,
    "omega_registry_version": "Ω/1.0",
    "capabilities": ['governance', 'reasoning', 'trajectory_validation'],
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryCognitiveCompass.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryCognitiveCompass',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryCognitiveCompass', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
# --- SML ORGAN ADAPTER END ---

# --- SML COMPASS SPECIALIZATION START ---
def sml_record_trajectory(packet, note="Compass observed SML trajectory"):
    """Record governed trajectory observation in the packet history."""
    from SarahMemorySMLProtocol import sml_touch_packet
    return sml_touch_packet(packet, organ="SarahMemoryCognitiveCompass", action="trajectory", omega="Ω050", note=note)
# --- SML COMPASS SPECIALIZATION END ---
