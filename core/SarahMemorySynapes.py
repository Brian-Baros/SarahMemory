...
def run_rem_sandbox_trial(candidate: Dict[str, Any], *, snapshot: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if set(target_files) & {"SarahMemoryGlobals.py", "SarahMemoryARILE.py"}:
        result = {"trial_id":trial_id,"passed":False,"reason":"SarahMemoryGlobals.py/SarahMemoryARILE.py protected file target is immutable.","rollback_ready":False,"artifact_path":os.path.join(rem_dir, f"{trial_id}.json")}
    else:
...
# -----------------------------------------------------------------------------
# SARAHMEMORY v9 SEMANTIC ANSWER / SELF-TOKENIZATION LEDGER
# -----------------------------------------------------------------------------
# Synapes is the living-model/provenance organ. These helpers record accepted or
# candidate semantic answer traces as bounded dataset-ledger samples. They do not
# train, promote, mutate core files, or grant execution authority.

_SM_SEMANTIC_SAMPLE_SCHEMA = "SarahMemory.Synapes.semantic_answer_sample.v1"


def _sm_syn_hash_json(value: Any) -> str:
    try:
...
def record_semantic_answer_sample(
    *,
    raw_text: str,
    answer: str,
    ...
    dataset_id: str = "sm_semantic_fast_answers",
) -> Dict[str, Any]:
    """Record a governed semantic answer/tokenization trace in dataset_ledger.

    This is the bridge from AdvCU/SEL/QIST runtime meaning to the SarahMemory
    living-model body under data/models/SarahMemory and synapses.db.

    Safety contract:
      - records only metadata/content supplied by caller
      - writes only through existing dataset_ledger path
      - no model training, adapter promotion, shell, web, API, or filesystem mutation
        outside Synapes' existing model/governance ledger files
    ...
    sem = semantic_packet if isinstance(semantic_packet, dict) else {}
    sel = sel_packet if isinstance(sel_packet, dict) else {}
    qist = qist_result if isinstance(qist_result, dict) else {}
    src = source if isinstance(source, dict) else {}

    if not raw or not ans:
        return {"ok": False, "recorded": False, "reason": "empty_raw_or_answer"}

    normalized = re.sub(r"\s+", " ", raw.lower()).strip()
    content = {
        "schema": _SM_SEMANTIC_SAMPLE_SCHEMA,
        "raw_text": raw,
    ...
    sample_hash = _sm_syn_hash_json(content)
    try:
        initialize_synapses_database()
    except Exception:
        pass
    try:
        ledger_hash = log_dataset_sample(
            dataset_id=dataset_id,
            sample_type="semantic_answer" if approved_for_learning else "semantic_answer_candidate",
            source_ref=f"semantic://{sample_hash}",
            content=content,
            score=max(0.0, min(1.0, float(confidence or 0.0))),
            verified=bool(approved_for_learning),
        )
        return {
            "ok": True,
            "recorded": True,
        ...
        return {"ok": False, "recorded": False, "error": str(exc), "sample_hash": sample_hash}


def record_self_tokenization_sample(
    *,
    raw_text: str,
    semantic_tokens: Optional[Dict[str, Any]] = None,
    tokenizer_profile_id: str = "sarahmemory_self_tokenizer_v0_1",
    sel_packet: Optional[Dict[str, Any]] = None,
    qist_result: Optional[Dict[str, Any]] = None,
    approved_for_learning: bool = False,
) -> Dict[str, Any]:
    """Record a semantic-token trace for future SarahMemory tokenizer growth."""
    content = {
        "schema": "SarahMemory.Synapes.self_tokenization_sample.v1",
        "raw_text": str(raw_text or ""),
        "normalized_text": re.sub(r"\s+", " ", str(raw_text or "").lower()).strip(),
        "semantic_tokens": semantic_tokens if isinstance(semantic_tokens, dict) else {},
        "tokenizer_profile_id": str(tokenizer_profile_id or "sarahmemory_self_tokenizer_v0_1"),
    ...
    try:
        ledger_hash = log_dataset_sample(
            dataset_id="sm_self_tokenization",
            sample_type="self_tokenization" if approved_for_learning else "self_tokenization_candidate",
            source_ref=f"selftoken://{sample_hash}",
            content=content,
            score=0.85 if approved_for_learning else 0.45,
            verified=bool(approved_for_learning),
        )
        return {
            "ok": True,
...
def get_living_model_dataset_status(dataset_id: str = "sm_semantic_fast_answers") -> Dict[str, Any]:
    """Return a small status summary for semantic/tokenization dataset ledger rows."""
    try:
        ensure_sarahmemory_model_dirs()
        initialize_synapses_database()
        conn = connect_db("synapses.db")
        cur = conn.cursor()
        cur.execute("SELECT sample_type, COUNT(1) FROM dataset_ledger WHERE dataset_id=? GROUP BY sample_type", (str(dataset_id),))
        counts = {str(r[0]): int(r[1]) for r in cur.fetchall()}
        cur.execute("SELECT COUNT(1) FROM dataset_ledger WHERE dataset_id=? AND verified=1", (str(dataset_id),))
        verified = int((cur.fetchone() or [0])[0] or 0)
        conn.close()
        return {
            "ok": True,
            ...
            "counts": counts,
            "verified": verified,
            "model_dir": SARAHMEMORY_MODEL_DIR,
            "datasets_dir": SARAHMEMORY_DATASETS_DIR,
            "execution_authority": False,
        }
    except Exception as exc:
        return {"ok": False, "dataset_id": str(dataset_id), "error": str(exc), "execution_authority": False}
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemorySynapes.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemorySynapes', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
