...
class SarahMemoryOperatorCore:
    ...
    def _energetics_preflight(self, contract: ActionContract) -> Dict[str, Any]:
        """Ask Energetics whether the organism can afford this action before dispatch.

        OperatorCore is a controlled motor/action layer. It must not fail open when
        Energetics is missing or corrupted for hazardous-energy-sensitive actions.
        SarahMemoryGlobals is checked first so ENERGETICS=True alone cannot arm
        execution without body-awareness, integrity, power, and operator verdicts.
        """
        ctx = {
            "action_type": contract.action_type,
            "phase": contract.primary_lane,
            "risk_level": contract.risk_level,
            "capability_name": contract.capability_name,
            "execution_mode": contract.execution_mode,
            "scheduled": contract.action_type in {"diagnostics", "evolution", "network_scan", "sync", "device_scan"},
        }
        try:
            blocker_fn = getattr(config, "sm_hazardous_energy_blocks_action", None)
            status_fn = getattr(config, "sm_hazardous_energy_status", None)
            if callable(blocker_fn) and blocker_fn(contract.action_type, ctx):
                status = status_fn(ctx) if callable(status_fn) else {}
                return {
                    "ok": False,
                    "decision": "DENY",
                    "reason": "OperatorCore blocked hazardous-energy-sensitive action by constitution preflight.",
                    "allowed_power_mode": "LOW_POWER",
                    "constitution": status,
                    "source": "SarahMemoryOperatorCore._energetics_preflight",
                }
            # When the constitution is in lockout but this contract is not classified as hazardous-energy-sensitive,
            # do not consult Energetics. Consulting the locked-out organ would return DENY and accidentally block
            # ordinary non-power/non-actuation workflows. Normal SafetyPolicies still review the action below.
            if callable(status_fn):
                status = status_fn(ctx)
                if isinstance(status, dict) and bool(status.get("lockout_active", True)):
                    return {
                        "ok": True,
                        "decision": "ALLOW",
                        "reason": "Energetics locked out; non-hazardous action bypasses Energetics and remains under normal governance.",
                        "allowed_power_mode": "LOW_POWER",
                        "constitution": status,
                        "source": "SarahMemoryOperatorCore._energetics_preflight",
                    }
            ...
            return {
                "ok": False,
                "decision": "DENY",
                "reason": f"OperatorCore hazardous-energy constitution check failed closed: {exc}",
                "allowed_power_mode": "LOW_POWER",
                "source": "SarahMemoryOperatorCore._energetics_preflight",
            }

        if _Energetics is None:
            return {"ok": True, "decision": "ALLOW", "reason": "Energetics unavailable; action is not classified as hazardous-energy-sensitive.", "allowed_power_mode": "ACTIVE"}
        try:
            ...
            if callable(fn):
                return fn(
                    contract.action_type,
                    phase=contract.primary_lane,
                    requested_power_mode="FULL_POWER" if contract.execution_mode == MODE_APPLY else "ACTIVE",
                    active_task=(contract.metadata or {}).get("active_task") if isinstance((contract.metadata or {}).get("active_task"), dict) else {},
                    internal_state=(contract.metadata or {}).get("internal_state") if isinstance((contract.metadata or {}).get("internal_state"), dict) else {},
                    external_state=(contract.metadata or {}).get("external_state") if isinstance((contract.metadata or {}).get("external_state"), dict) else {},
                    constraints={
                        "risk_level": contract.risk_level,
                        "execution_mode": contract.execution_mode,
                        "capability_name": contract.capability_name,
                        "operator_core_contract": True,
                    },
                    context=ctx,
                )
        except Exception as exc:
            return {"ok": False, "decision": "DENY", "reason": f"Energetics preflight failed closed for governed OperatorCore path: {exc}", "allowed_power_mode": "LOW_POWER"}
        return {"ok": True, "decision": "ALLOW", "reason": "Energetics bridge missing; action is not classified as hazardous-energy-sensitive.", "allowed_power_mode": "ACTIVE"}
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryOperatorCore.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryOperatorCore',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryOperatorCore', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
# --- SML ORGAN ADAPTER END ---

# --- SML OPERATORCORE SPECIALIZATION START ---
def sml_execution_gate(packet):
    """Return whether an SML packet is approved for OperatorCore execution."""
    from SarahMemorySMLProtocol import SMLPacket, GovernanceDecision, Authority
    pkt = packet if isinstance(packet, SMLPacket) else SMLPacket.from_dict(packet)
    decision = str((pkt.governance or {}).get("decision") or "")
    granted = set((pkt.authority or {}).get("granted") or [])
    required = set((pkt.authority or {}).get("required") or [])
