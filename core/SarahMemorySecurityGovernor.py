...
SML_ORGAN_METADATA = {
    "name": 'SarahMemorySecurityGovernor',
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemorySecurityGovernor.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemorySecurityGovernor',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemorySecurityGovernor', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
# --- SML ORGAN ADAPTER END ---

# --- SML SECURITY GOVERNOR SPECIALIZATION START ---
def sml_security_review_packet(packet, note="SecurityGovernor reviewed SML packet"):
    """Record security-governor observation in the SML packet."""
