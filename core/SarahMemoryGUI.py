...
class SarahMemoryGUI:
    ...
    def shutdown(self):
        if getattr(self, "_shutdown_in_progress", False):
            return
        if not messagebox.askokcancel("Exit", "Are you sure you want to exit?"):
            logger.info("Shutdown cancelled by user.")
            return
        self._shutdown_in_progress = True
        log_gui_event("Shutdown", "User exited the GUI.")

        # Cancel scheduled callbacks before destroying Tk widgets. Never schedule
        # replacement callbacks during shutdown.
        jobs = []
        for name in ("video_update_job", "status_update_job", "avatar_update_job"):
            jobs.append(getattr(self, name, None))
        for name in ("update_video_job_id", "update_status_job_id", "update_avatar_job_id"):
            jobs.append(globals().get(name))
        for job in jobs:
            if job:
                try:
        ...
        try:
            ...
            pass
        try:
            proc = getattr(self, "avatar_proc", None)
        ...
        try:
            ...
            terminate = getattr(_Integration, "terminate_flag", None)
            if terminate is not None:
                terminate.set()
            cleanup = getattr(_Integration, "_call_main_process_cleanup", None)
            if callable(cleanup):
                cleanup()
        except Exception:
            pass
        ...
        try:
            ...
            pass
        logger.info("SarahMemory GUI shutdown completed gracefully.")
    ...
    if bool(getattr(config, "USE_WEBVIEW", False)) and ui_mode in ("web", "custom"):
        try:
            ...
            if ui_mode == "custom":
                ...
                # V9 custom UI is served from BASE_DIR/data/ui/v9. Do not load
                # BASE_DIR/ui/v9 or file:// paths; those caused blank webview
                # behavior when Vite assets used absolute /assets URLs.
                base_dir = getattr(config, "BASE_DIR", os.getcwd())
                html_path = getattr(config, "CUSTOM_UI_INDEX", os.path.join(base_dir, "data", "ui", "v9", "index.html"))
            elif ui_mode == "web":
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryGUI.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryGUI', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
