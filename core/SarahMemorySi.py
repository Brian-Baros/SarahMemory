...
def _process_matches_app(process_info: Dict[str, Any], app_name: str) -> bool:
    """Match executable identity exactly; never search arbitrary command text."""
    canonical = _normalize_app_alias(app_name)
    expected = {canonical.lower(), f"{canonical.lower()}.exe"}
    raw = str(app_name or "").strip().lower()
    if raw:
        expected.update({raw, raw if raw.endswith(".exe") else f"{raw}.exe"})
    name = os.path.basename(str(process_info.get('name') or '')).lower()
    exe_name = os.path.basename(str(process_info.get('exe') or '')).lower()
    cmdline = process_info.get('cmdline') or []
    first_arg = os.path.basename(str(cmdline[0] or '')).lower() if isinstance(cmdline, (list, tuple)) and cmdline else ""
    identities = {item for item in (name, exe_name, first_arg) if item}
    return bool(identities.intersection(expected))
...
def _find_running_processes(app_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        for proc in psutil.process_iter(attrs=['pid', 'name', 'exe', 'cmdline', 'status']):
            try:
                if not _process_matches_app(proc.info, app_name):
                    continue
                matches.append({
...
def execute_play_command(action: str, target: str, original_query: str) -> str:
    ...
    try:
        ...
        # Local media player: open only a verified local file path. SarahMemory
        # does not crawl drives or guess media locations on this path.
        elif "media player" in platform:
            candidate = os.path.abspath(os.path.expanduser(str(song or "").strip().strip('"')))
            if not os.path.isfile(candidate):
                return f"Local media file was not found at the explicitly supplied path: {candidate}"
            try:
                if os.name == "nt":
                    os.startfile(candidate)  # type: ignore[attr-defined]
                elif sys.platform == "darwin":
                    subprocess.Popen(["open", candidate], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                else:
                    subprocess.Popen(["xdg-open", candidate], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                logger.info("[v8.0] Local media playback requested: %s", candidate)
                return f"Opened local media file: {candidate}"
            except Exception as media_exc:
                logger.error("[v8.0] Local media player launch failed: %s", media_exc)
                return f"Local media player could not open the verified file: {media_exc}"
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemorySi.py'},
}
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemorySi', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
