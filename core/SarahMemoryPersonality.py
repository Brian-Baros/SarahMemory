...
def connect_personality_db() -> Optional[sqlite3.Connection]:
    ...
    try:
        ...
        # Connect remains local SQLite only. The schema healer runs immediately
        # after opening the DB so boot-time reads never spam "no such table:
        # interactions" and personality logging remains auditable.
        os.makedirs(os.path.dirname(DB_PATH), exist_ok=True)
        conn = sqlite3.connect(DB_PATH, timeout=10.0)
        conn.row_factory = sqlite3.Row  # Enable dict-like access
        _ensure_personality_runtime_schema(conn)
...
# SARAHMEMORY_PATCH_NOTE 2026-06-23:
# Personality boot noise fix and auditability hardening.
# The V8/V9 personality engine reads both "responses" and "interactions" during
# normal startup. Older databases may exist without the "interactions" table,
# causing repeated "no such table: interactions" errors during boot and GUI
# launch. This healer creates only the missing tables/indexes and never deletes,
# renames, or overwrites existing rows. It is intentionally small and local-only.
# It preserves backwards compatibility with existing personality1.db data while
# ensuring every processed interaction remains auditable.
_PERSONALITY_SCHEMA_READY = False

def _ensure_personality_runtime_schema(conn: Optional[sqlite3.Connection] = None) -> bool:
    """Ensure local personality tables exist without destructive migration."""
    global _PERSONALITY_SCHEMA_READY
    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS responses (
                ...
                source TEXT DEFAULT 'runtime_schema_healer',
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS interactions (
                ...
                source TEXT DEFAULT 'SarahMemoryPersonality'
            )
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_personality_interactions_timestamp ON interactions(timestamp)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_personality_interactions_intent ON interactions(intent)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_personality_responses_intent ON responses(intent)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_personality_responses_tone ON responses(tone)")
        conn.commit()
        _PERSONALITY_SCHEMA_READY = True
        return True
    except Exception as e:
        logger.warning(f"Personality runtime schema healer failed: {e}")
        return False
    finally:
        if owns_connection:
            try:
...
def ensure_personality_runtime_schema() -> bool:
    """Public diagnostics hook for boot-time schema readiness checks."""
    return _ensure_personality_runtime_schema()
...
def get_identity_response(user_input: Optional[str] = None) -> str:
    """Return the governed active identity without statistical selection."""
    try:
        import SarahMemoryCognitiveSelf as _CogSelf  # type: ignore
        resolver = getattr(_CogSelf, "resolve_active_identity", None)
        if callable(resolver):
            identity = resolver({"request_text": str(user_input or "")}) or {}
            if isinstance(identity, dict):
                name = str(identity.get("active_name") or identity.get("name") or "Sarah").strip() or "Sarah"
                return f"I'm {name} — your SarahMemory AiOS companion."
    except Exception as exc:
        logger.debug("CognitiveSelf identity resolution unavailable: %s", exc)
    try:
        response = get_reply_from_db("identity")
        if isinstance(response, str) and response.strip():
            return response.strip()
    except Exception as exc:
        logger.debug("Identity database response unavailable: %s", exc)
...
def log_personality_interaction(interaction: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        _ensure_personality_runtime_schema(conn)
        cursor.execute("""
            INSERT INTO interactions (timestamp, intent, response, user_input, metadata_json, source)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            ...
            json.dumps(interaction.get('metadata', {}), ensure_ascii=False, default=str),
            'SarahMemoryPersonality.log_personality_interaction'
        ))
...
def update_personality_model() -> Dict[str, float]:
    ...
    try:
        ...
        logger.warning(f"Personality model update degraded safely: {e}")
...
try:
    # SARAHMEMORY_PATCH_NOTE 2026-06-23:
    # Non-destructive boot-time personality schema readiness check.
    # This is local-only and intentionally does not seed or overwrite data.
    _ensure_personality_runtime_schema()
except Exception:
    pass
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryPersonality.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryPersonality',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryPersonality', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
