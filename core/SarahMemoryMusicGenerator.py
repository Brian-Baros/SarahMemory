...
def ensure_music_directories() -> None:
    """Create music workspace directories only when the studio is explicitly initialized."""
    for directory in [MUSIC_DIR, MUSIC_PROJECTS_DIR, MUSIC_CACHE_DIR, EXPORTS_DIR,
                      MUSIC_SAMPLES_DIR, MUSIC_PRESETS_DIR, MUSIC_LOOPS_DIR,
                      MUSIC_RECORDINGS_DIR, MUSIC_MIDI_DIR]:
        try:
            os.makedirs(directory, exist_ok=True)
        except Exception as exc:
            logging.warning(f"[MusicStudio] Could not create directory {directory}: {exc}")
...
class MusicStudio:
    ...
    def __init__(self, sample_rate: int = DEFAULT_SAMPLE_RATE):
        """Initialize Music Studio with a bounded, supported sample rate."""
        ...
        if resolved_rate not in SUPPORTED_SAMPLE_RATES:
            raise ValueError(f"Unsupported sample_rate: {resolved_rate}; supported={SUPPORTED_SAMPLE_RATES}")
        ...
        logging.info(f"[MusicStudio] Sample Rate: {self.sample_rate}Hz")
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        """Return a truthful, read-only music-engine capability report."""
        return {
            "ok": True,
            "module": "SarahMemoryMusicGenerator",
            "version": MUSIC_STUDIO_VERSION,
    ...
    def build_output_manifest(self, project: MusicProject, output_path: str = "") -> Dict[str, Any]:
        """Build a bounded music-output manifest without rendering a file."""
        return {
            "schema": "SARAHMEMORY_MUSIC_OUTPUT_V1",
            "project_id": getattr(project, "project_id", ""),
            "name": getattr(project, "name", ""),
            "tempo": float(getattr(project, "tempo", 0.0) or 0.0),
            "key": str(getattr(project, "key", "")),
            "sample_rate": int(getattr(project, "sample_rate", self.sample_rate) or self.sample_rate),
    ...
    def enterprise_self_test(self) -> Dict[str, Any]:
        """Run bounded in-memory synthesis invariants without writing audio."""
        checks: List[Dict[str, Any]] = []
        tone = generate_tone(440.0, 0.02, self.sample_rate)
        expected = max(1, int(0.02 * self.sample_rate)) * 2  # 16-bit mono bytes
        checks.append({"name": "tone_length", "passed": len(tone) == expected, "observed": len(tone)})
        checks.append({"name": "tone_bytes", "passed": isinstance(tone, (bytes, bytearray)), "observed": type(tone).__name__})
        note = MusicTheory.midi_to_freq(69)
        checks.append({"name": "midi_a4", "passed": abs(note - 440.0) < 1e-6, "observed": note})
        project = MusicProject("self_test", "EnterpriseSelfTest", 120.0, "C", (4, 4), self.sample_rate)
        manifest = self.build_output_manifest(project)
        checks.append({"name": "manifest_contract", "passed": manifest.get("schema") == "SARAHMEMORY_MUSIC_OUTPUT_V1", "observed": manifest})
...
def get_music_generator_capabilities() -> Dict[str, Any]:
    """Read-only module capability report without creating directories or projects."""
    return {
        "ok": True,
        "module": "SarahMemoryMusicGenerator",
        "version": MUSIC_STUDIO_VERSION,
...
SML_ORGAN_METADATA = {
    "name": 'SarahMemoryMusicGenerator',
    ...
    "internal_only": True,
    "metadata": {"sml_adapter": "generic_non_executing", "source_file": 'SarahMemoryMusicGenerator.py'},
}
...
def sml_diagnostics():
    ...
    return {
        "status": "OK",
        "component": 'SarahMemoryMusicGenerator',
        "sml_adapter": True,
...
def sml_receive_packet(packet, *, action="observe", note="", updates=None):
    ...
    try:
        ...
        return sml_touch_packet(packet, organ='SarahMemoryMusicGenerator', action=action, note=note or "organ observed packet", updates=updates)
    except Exception:
        return packet
