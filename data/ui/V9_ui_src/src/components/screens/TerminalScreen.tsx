...
function stripTerminalPrefix(raw: string): { route: "shell" | "ai"; command: string; original: string } {
  ...
  const command = explicitShell
    ...
    : explicitAi
      ...
      : original;
  const route: "shell" | "ai" = explicitShell ? "shell" : explicitAi ? "ai" : looksLikeShellCommand(original) ? "shell" : "ai";
  return { route, command, original };
}
...
export default function TerminalScreen() {
  ...
  const frontendDevMode = Boolean((store as any)?.developersMode ?? (store as any)?.DEVELOPERSMODE ?? false);
  const desktopAllowed = !isMobile;
  // The frontend must not blank the terminal just because local UI state is not
  // synchronized with SarahMemoryGlobals.py. Backend DEVELOPERSMODE remains the authority.
  ...
  const tryBackendCall = useCallback(async (path: string, payload?: any) => {
    try {
      if (path === "/api/terminal/status") {
        return await api.terminal.status(payload || {});
      }
      if (path === "/api/terminal/execute") {
        return await api.terminal.execute(payload || {});
      }
      if (path === "/api/terminal/agent") {
        return await api.terminal.agent(payload || {});
      }
      if (path === "/api/chat") {
        const message = String(payload?.message || payload?.input || payload?.text || "");
        return await api.chat.sendMessage([{ role: "user", content: message }], {
          intent: "terminal_agent_task",
          tone: "technical",
          complexity: "adult",
        });
      }
      ...
      try {
        ...
        const response =
          terminalRequest.route === "shell"
            ? await tryBackendCall("/api/terminal/execute", {
                command: terminalRequest.command,
                query: terminalRequest.command,
                source: "terminal_screen",
                mode: "auto",
                session_id: "terminal_screen",
                caller: "WebUI.TerminalScreen",
                // Explicit local-user confirmation for the backend governance preflight.
                // This only acknowledges the user's Run action; backend policy still decides.
                confirmed: true,
                user_confirmed: true,
                confirm_phrase: "APPROVE GOVERNED ACTION",
              })
            : explicitAgentRequest
              ? await tryBackendCall("/api/terminal/agent", {
                  task: terminalRequest.command,
                  text: terminalRequest.command,
                  source: "terminal_screen",
              ...
              : await tryBackendCall("/api/chat", {
                  message: terminalRequest.command,
                  input: terminalRequest.command,
                  source: "terminal_screen",
                  mode: "governed_ai_task",
                });

        if (response) {
          setBackendAvailable(true);

          const blocked = Boolean((response as any)?.blocked);
          const stdout = normalizeText((response as any)?.stdout);
          const stderr = normalizeText((response as any)?.stderr);
          const reason = normalizeText((response as any)?.reason || (response as any)?.error);
          const reply =
            stdout ||
            stderr ||
            reason ||
            ...
            normalizeText((response as any)?.content) ||
            "Command processed.";

          pushLog({
            level: blocked ? "warning" : terminalRequest.route === "shell" ? "success" : "info",
            source: terminalRequest.route === "shell" ? "terminal" : "ai",
            text:
              terminalRequest.route === "shell"
                ? [
                    stdout ? `STDOUT:
...
${stderr}` : "",
                    reason ? `REASON:
${reason}` : "",
                    `exit_code=${(response as any)?.exit_code ?? "n/a"}`,
                  ]
                    .filter(Boolean)
                    .join("\n\n")
                : reply,
          ...
          setStatusText(terminalRequest.route === "shell" ? "Terminal command completed." : "AI/backend task completed.");
        } else {
          ...
          pushLog({
            ...
            source: "fallback",
            text:
              "No backend terminal/chat endpoint responded. The request was not completed.",
          });
    ...
    return (
      <div className="w-full h-full p-3 flex items-center justify-center">
        <div className="rounded-xl border border-border bg-card p-6 max-w-md text-center">
          ...
          <p className="text-xs text-muted-foreground mt-2">The developer CLI surface is desktop-only.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full p-3">
      <div className="grid grid-cols-1 xl:grid-cols-[360px_minmax(0,1fr)] gap-3 h-full">
        {/* Left Rail */}
        <div className="hidden xl:flex min-h-0 rounded-xl border border-border bg-card overflow-hidden flex-col">
            ...
            <div className="mt-3 space-y-2 text-xs">
              <div className="flex items-center justify-between rounded-lg border border-border bg-background/50 px-3 py-2">
                ...
                <span className="text-foreground font-medium">
                  {backendAvailable === true ? "Backend enabled" : frontendDevMode || uiMode === "engineer" ? "UI enabled" : "Backend-gated"}
                </span>
              </div>
