...
export function FilesScreen() {
  ...
  const [surface, setSurface] = useState<FileSurface>("files");
...
if (a.type === "files_open_trash" || a.type === "trash_open") {
  void openTrashView();
}
if (a.type === "files_preview" || a.type === "files_open") {
    ...
    if (hit) {
      ...
      if (a.type === "files_preview") void openFilePreview(it);
        ...
        const pendingTimer = window.setTimeout(() => {
          try {
            const raw = window.sessionStorage.getItem(FILES_PENDING_KEY);
            if (!raw) return;
            window.sessionStorage.removeItem(FILES_PENDING_KEY);
            const actions = JSON.parse(raw);
            if (Array.isArray(actions) && actions.length) {
              handler({ detail: { actions, source: "files-pending" } });
            }
          } catch {
            // best-effort pending action replay
          }
        }, 0);
        return () => {
  ...
  async function openTrashView(capabilityOverride?: Capabilities | null) {
    setSurface("trash");
    setCwd("trash:///");
    setItems([]);
    clearSelection();

    const trashCapable = capabilityOverride ?? capsRef.current ?? caps;
    if (!trashCapable?.canTrash) {
      toast.message("Trash is waiting for backend dumpster endpoints.");
      return;
    }

    const candidates = [
      "/api/files/trash/list",
      "/api/files/dumpster/list",
      "/api/files/list_trash",
    ];
    ...
    try {
      for (const url of candidates) {
        try {
          const data = await apiPost<{ ok: boolean; path?: string; items?: FileItem[] }>(url, {});
          setItems(data.items || []);
          setCwd(data.path || "trash:///");
          return;
        } catch (e) {
          lastError = e;
        }
      }
      throw lastError || new Error("Trash list endpoint not available");
    } catch (e: any) {
      toast.error(e?.message || "Trash service is not available yet");
    } finally {
      setLoading(false);
    }
  }
  ...
  const actionsDisabled = !caps?.canBrowse && surface !== "trash";
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden bg-background text-foreground md:flex-row">
      {/* Sidebar */}
      <div className="hidden w-64 shrink-0 border-r border-border/60 p-3 md:block">
        ...
        <div className="space-y-2">
          <SidebarItem
            ...
            label="This PC"
            active={surface === "files" && cwd === "/"}
            onClick={() => caps?.canBrowse && list("/")}
          />
          <SidebarItem
            icon={<Trash2 className="h-4 w-4" />}
            label="Trash"
            rightText={!canTrash ? "Bridge pending" : ""}
            active={surface === "trash"}
      ...
      <div className="shrink-0 border-b border-border/60 bg-muted/10 p-2 md:hidden">
        <div className="flex gap-2 overflow-x-auto">
          <Button
            variant={surface === "files" && cwd === "/" ? "default" : "outline"}
            size="sm"
          ...
          <Button
            variant={surface === "trash" ? "default" : "outline"}
            size="sm"
            className="h-10 min-w-24 gap-2"
            onClick={() => openTrashView()}
          >
          ...
          <Button
            ...
            disabled={!caps?.canBrowse || surface === "trash"}
          >
      ...
      >
        ...
        <div className="border-b border-border/60 p-3 flex flex-wrap items-center gap-2">
          ...
          {/* Breadcrumb */}
          <div className="flex min-w-[180px] max-w-[520px] flex-1 items-center gap-1 overflow-hidden rounded-md border border-border/60 bg-muted/20 px-2 py-1 sm:min-w-[260px]">
            {(surface === "trash" ? ["Trash"] : crumbs).map((c, i) => (
              <React.Fragment key={i}>
                <button
                  className="text-sm hover:underline truncate max-w-[160px]"
                  onClick={() => (surface === "trash" ? openTrashView() : list(crumbToPath(i)))}
                >
                  {c}
                </button>
                {surface !== "trash" && i < crumbs.length - 1 && <ChevronRight className="h-4 w-4 text-muted-foreground" />}
              </React.Fragment>
            ))}
          </div>
          ...
          <Button
            ...
            disabled={!caps?.canMkdir || surface === "trash"}
            title="New Folder"
          >
        ...
        {/* Body */}
        <div className="flex-1 min-h-0 overflow-hidden p-3 flex flex-col gap-3 xl:flex-row">
          {/* File list panel */}
          <div className="flex-1 min-w-0 min-h-0">
            {!caps?.canBrowse && surface !== "trash" ? (
              ...
              <div className="min-h-0 rounded-lg border border-border/60 overflow-hidden">
                {surface === "trash" && !canTrash && (
                  <div className="border-b border-border/60 bg-muted/20 p-3 text-sm text-muted-foreground">
                    Trash is wired in the UI, but this backend has not advertised dumpster/trash endpoints yet.
                    Move-to-Trash will activate when the bridge returns <code className="rounded bg-background/60 px-1">canTrash: true</code>.
                  </div>
                )}
                {viewMode === "details" && (
                  <>
                    <div className="min-w-[760px] grid grid-cols-[min-content_1fr_120px_140px_200px] gap-2 px-3 py-2 bg-muted/30 text-xs font-semibold border-b border-border/60">
                      <div className="w-8" />
                        ...
                        sortedItems.map((it) => {
                          ...
                          return (
                            <div
                              key={it.path}
                              className={cn(
                                "min-w-[760px] grid grid-cols-[min-content_1fr_120px_140px_200px] gap-2 px-3 py-2 text-sm border-b border-border/40 cursor-default select-none",
                ...
                {viewMode === "icons" && (
                  <div className="p-3 max-h-[calc(100dvh-180px)] overflow-auto">
                    {loading ? (
                      <div className="p-6 text-sm text-muted-foreground">Loading…</div>
                    ) : sortedItems.length === 0 ? (
                      <div className="p-6 text-sm text-muted-foreground">No items</div>
          ...
          {propsOpen && propsItem && (
            <div className="w-full shrink-0 border border-border/60 rounded-lg overflow-hidden bg-muted/10 xl:w-[340px]">
        ...
        {/* Footer */}
        <div className="border-t border-border/60 p-2 text-xs text-muted-foreground flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <span>
