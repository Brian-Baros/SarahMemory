...
def ensure_repo() -> None:
    ...
    if not git_dir.exists():
        ...
        # REPO_DIR already ensured by ensure_base_dirs(), but mkdir for safety
...
def ensure_node_modules(skip_install: bool = False) -> None:
    """
    Install npm dependencies if node_modules is missing or if forced.
    """
    node_modules = REPO_DIR / "node_modules"

    if skip_install and node_modules.exists():
        print("[INFO] Skipping npm install (requested) and node_modules exists.")
        return
    ...
    run_cmd("npm install", cwd=REPO_DIR)
...
def build_frontend(skip_build: bool = False, build_script: str = "build") -> None:
    ...
    print(f"[INFO] Running npm run {build_script} in {REPO_DIR}...")
    run_cmd(f"npm run {build_script}", cwd=REPO_DIR)
    print("[INFO] Frontend build complete.")


def locate_build_output(custom_build_dir: str | None = None) -> Path:
    """
    Locate the built static output directory (dist or build or custom) inside REPO_DIR.
    """
    candidates: list[Path] = []

    if custom_build_dir:
        candidates.append(REPO_DIR / custom_build_dir)
...
def main() -> int:
    ...
    print(f"[INFO] REPO_URL    = {REPO_URL}")
    print("------------------------------------------------------------")
