...
def _get_export_root() -> str:
    ...
    # last-resort: derive from api/server path instead of process cwd
    try:
        here = Path(__file__).resolve()
        root_base = here.parents[2]
    except Exception:
        root_base = Path(os.getcwd()).resolve()
    root = os.path.join(os.fspath(root_base), "data", DEFAULT_EXPORT_SUBDIR)
    _ensure_dir(root)
    return root
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": False,
    "metadata": {"sml_adapter": "api_bridge_non_executing", "source_file": 'appmedia.py'},
}
