...
# SARAHMEMORY_PATCH_NOTE: local portable download hint. Never hardcode a Windows drive/root; use env or SarahMemoryGlobals when available.
try:
    import SarahMemoryGlobals as _sm_cfg_for_net  # type: ignore
    _sm_default_downloads = getattr(_sm_cfg_for_net, "DOWNLOADS_DIR", os.path.join(getattr(_sm_cfg_for_net, "BASE_DIR", os.getcwd()), "downloads"))
except Exception:
    _sm_default_downloads = os.path.join(os.getcwd(), "downloads")
SUGGESTED_CLIENT_DOWNLOAD_DIR = os.environ.get("SARAHNET_SUGGESTED_CLIENT_DOWNLOAD_DIR", _sm_default_downloads)
...
def _verify_broker_auth(body_bytes: bytes) -> bool:
    ...
    # SARAHMEMORY_PATCH_NOTE: broker dev fallback is loopback-only.
    # A missing injected auth handler must not allow outside hosts or AI agents
    # to queue files/commands into the broker. Local UI/dev calls from loopback remain usable.
    try:
        remote_addr = (getattr(request, "remote_addr", "") or "").strip()
...
def _sarahnet_payload_hijack_marker(data: Dict[str, Any]) -> bool:
    """Detect obvious authority-inversion payloads at the broker edge.

    # SARAHMEMORY_PATCH_NOTE: sync channel anti-agent hijack gate. Broker storage
    # never executes packets, but it should still reject attempts that label model
    # or agent text as authority to overwrite core files.
    """
    try:
        ...
        text = str(data).lower()
    markers = (
        "ignore governance", "bypass governance", "self-authorize", "self authorize",
        "execute without approval", "disable audit", "disable safety", "overwrite core",
        "apply core patch", "remote shell", "ai agent authority",
    )
...
def net_file_send_small():
    ...
    # SARAHMEMORY_PATCH_NOTE: reject obvious AI-agent hijack/instruction payloads before broker storage.
    if _sarahnet_payload_hijack_marker(data):
        _arile_api_emit("anti_agent_hijack_denied", "SarahNet file/send rejected an authority-inversion payload.", severity=0.82, from_node=from_node, to_node=to_node, filename=filename)
        return _err("Rejected by anti-agent hijack gate", 403, error_code="anti_agent_hijack_denied")
...
# ---------------------------------------------------------------------
# Energetics network/device power governance endpoints
# ---------------------------------------------------------------------
@bp.get("/api/net/energetics/status")
def net_energetics_status():
    # Status must remain visible during lockout so operators can see exactly why
    # Energetics has no authority. Missing Energetics falls back to the constitution status.
    ctx = {"source": "appnet.status", "domain": "network"}
    try:
        ...
        try:
            ...
            pass
        return _err("energetics_status_error", 500, detail=str(exc))
    try:
        import SarahMemoryGlobals as _SMG  # type: ignore
        status_fn = getattr(_SMG, "sm_hazardous_energy_status", None)
        if callable(status_fn):
...
def net_energetics_preflight():
    ...
    data = _j()
    ctx = {
        "source": "appnet.preflight",
        "domain": "network",
        "action_type": str(data.get("action_type") or data.get("action") or "network_action"),
        "requested_power_mode": str(data.get("requested_power_mode") or "ACTIVE"),
        "device_type": str(data.get("device_type") or "network"),
    }
    try:
        import SarahMemoryGlobals as _SMG  # type: ignore
        blocker_fn = getattr(_SMG, "sm_hazardous_energy_blocks_action", None)
        status_fn = getattr(_SMG, "sm_hazardous_energy_status", None)
        if callable(blocker_fn) and blocker_fn("network_device_power", ctx):
            ...
            return jsonify({
                "ok": False,
                "decision": "DEFER",
                "reason": "Hazardous-energy constitution blocks network Energetics preflight from granting authority.",
                "allowed_power_mode": "LOW_POWER",
                "constitution": status,
            }), 200
    except Exception as exc:
        return jsonify({"ok": False, "decision": "DEFER", "reason": f"Network Energetics constitution check failed closed: {exc}", "allowed_power_mode": "LOW_POWER"}), 200
    try:
        import SarahMemoryEnergetics as _Energetics  # type: ignore
        ...
        if callable(fn):
            verdict = fn(
                ...
                requested_power_mode=ctx["requested_power_mode"],
                device_type=ctx["device_type"],
                active_task=data.get("active_task") if isinstance(data.get("active_task"), dict) else {},
                internal_state=data.get("internal_state") if isinstance(data.get("internal_state"), dict) else {},
                external_state=data.get("external_state") if isinstance(data.get("external_state"), dict) else {},
                constraints=data.get("constraints") if isinstance(data.get("constraints"), dict) else {},
            )
            return jsonify(verdict), 200
    except Exception as exc:
        return jsonify({"ok": False, "decision": "DEFER", "reason": f"Energetics network preflight failed closed: {exc}", "allowed_power_mode": "LOW_POWER"}), 200
    return jsonify({"ok": False, "decision": "DEFER", "reason": "Energetics unavailable; network preflight fails closed.", "allowed_power_mode": "LOW_POWER"}), 200
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": False,
    "metadata": {"sml_adapter": "api_bridge_non_executing", "source_file": 'appnet.py'},
}
