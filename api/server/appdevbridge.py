...
def _normalize_ticket_text(value: Any) -> str:
    text = re.sub(r"\s+", " ", str(value or "").strip().lower())
    text = re.sub(r"[^a-z0-9_./:\\ -]+", "", text)
    return text[:2000]


def _repair_ticket_fingerprint(record: Dict[str, Any], kind: str = "ticket") -> str:
    target = _normalize_ticket_text(record.get("target_file") or record.get("target_path") or record.get("file") or record.get("module") or "")
    issue = _normalize_ticket_text(record.get("issue") or record.get("title") or record.get("summary") or record.get("reason") or "")
    category = _normalize_ticket_text(record.get("category") or record.get("ticket_type") or record.get("batch_type") or kind)
    # Strip volatile identifiers/numbers so repeated diagnostics group together.
    issue_shape = re.sub(r"\b\d{2,}\b", "#", issue)
    return hashlib.sha256(f"{kind}|{target}|{category}|{issue_shape}".encode("utf-8", "ignore")).hexdigest()


def _repair_backlog_triage(limit: int = 2000) -> Dict[str, Any]:
    ...
    severity_counts: Dict[str, int] = {}
    target_counts: Dict[str, int] = {}
    records: List[Tuple[str, Dict[str, Any]]] = [("ticket", item) for item in tickets] + [("batch", item) for item in batches]
    for kind, item in records:
        status = str(item.get("status") or item.get("state") or "open").strip().lower()
        severity = str(item.get("severity") or item.get("risk") or "unspecified").strip().lower()
        target = str(item.get("target_file") or item.get("target_path") or item.get("file") or item.get("module") or "unassigned")[:500]
        status_counts[status] = status_counts.get(status, 0) + 1
        severity_counts[severity] = severity_counts.get(severity, 0) + 1
        target_counts[target] = target_counts.get(target, 0) + 1
        fp = _repair_ticket_fingerprint(item, kind)
        group = groups.setdefault(fp, {
            "fingerprint": fp,
            "kind": kind,
            "target": target,
            "issue_shape": str(item.get("issue") or item.get("title") or item.get("summary") or "")[:1000],
            "count": 0,
            "record_ids": [],
            "statuses": {},
            "severities": {},
        ...
        group["count"] += 1
        rid = str(item.get("ticket_id") or item.get("batch_id") or item.get("record_id") or "")
        if rid and len(group["record_ids"]) < 100:
        ...
        if item.get("_path") and len(group["paths"]) < 20:
            group["paths"].append(item.get("_path"))
    ordered = sorted(groups.values(), key=lambda g: (-int(g.get("count", 0)), str(g.get("target") or ""), str(g.get("fingerprint") or "")))
    duplicate_groups = [g for g in ordered if int(g.get("count", 0)) > 1]
    duplicate_records = sum(max(0, int(g.get("count", 0)) - 1) for g in duplicate_groups)
    priority = []
    rank = {"critical": 0, "high": 1, "medium": 2, "low": 3, "unspecified": 4}
    for group in ordered:
        top_severity = sorted(group.get("severities", {}).keys(), key=lambda x: rank.get(x, 5))[0] if group.get("severities") else "unspecified"
        priority.append({
            "fingerprint": group["fingerprint"], "target": group["target"], "count": group["count"],
            "top_severity": top_severity, "statuses": group["statuses"], "record_ids": group["record_ids"][:25],
            "recommended_disposition": "review_one_mark_remainder_duplicate" if group["count"] > 1 else "review_individually",
        })
    priority.sort(key=lambda x: (rank.get(x["top_severity"], 5), -int(x["count"]), str(x["target"])))
    return {
        "schema": "SARAHMEMORY_DEVBRIDGE_REPAIR_TRIAGE_V1",
        "generated_at": _iso(),
        "dry_run": True,
        "mutation_performed": False,
        "scanned": {"repair_tickets": len(tickets), "repair_batches": len(batches), "total": len(records), "limit_per_kind": limit},
        "status_counts": dict(sorted(status_counts.items())),
        "severity_counts": dict(sorted(severity_counts.items())),
        "top_targets": [{"target": k, "count": v} for k, v in sorted(target_counts.items(), key=lambda kv: (-kv[1], kv[0]))[:100]],
        "unique_fingerprints": len(ordered),
        "duplicate_groups": len(duplicate_groups),
        "duplicate_records_after_representative": duplicate_records,
        "priority_plan": priority[:500],
        "duplicate_group_preview": duplicate_groups[:100],
        "governance": {
            "auto_clear": False,
            "auto_apply": False,
            "requires_disposition_tickets": True,
            "user_final_authority": True,
            "note": "Triage groups evidence only. It never deletes, edits, or marks tickets automatically.",
        },
    }


@bp.get("/api/devbridge/repair/triage")
def devbridge_repair_triage():
    if not _auth_ok(_body_bytes()):
        return _err("Unauthorized", 401)
    limit = int(request.args.get("limit") or 2000)
    return _ok(triage=_repair_backlog_triage(limit))


@bp.post("/api/devbridge/repair/triage-plan")
def devbridge_repair_triage_plan():
    """Create a read-only triage plan; no ticket disposition is applied."""
    if not _auth_ok(_body_bytes()):
        return _err("Unauthorized", 401)
    if not _developer_mode_enabled():
        return _err("Developer mode is required", 403)
    data = _j()
    triage = _repair_backlog_triage(int(data.get("limit") or 2000))
    if _boolish(data.get("persist_plan"), False):
        if not _boolish(data.get("confirmed"), False):
            return _err("Explicit confirmation required to persist the triage plan", 403, triage=triage)
        plan_id = _new_id("triage")
        plan_path = os.path.join(_subdir("reports"), f"{plan_id}.json")
        _ensure_dir(os.path.dirname(plan_path))
        _write_json(plan_path, triage)
        _record("repair_triage", plan_id, "planned", data, triage, plan_path)
        return _ok(triage=triage, plan_id=plan_id, plan_path=plan_path, mutation="report_only")
    return _ok(triage=triage, mutation="none")
...
SML_ORGAN_METADATA = {
    ...
    "internal_only": False,
    "metadata": {"sml_adapter": "api_bridge_non_executing", "source_file": 'appdevbridge.py'},
}
